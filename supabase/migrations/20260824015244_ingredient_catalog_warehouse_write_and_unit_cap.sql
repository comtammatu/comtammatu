-- ADR 0045: warehouse catalog write authority.
-- 1) Seed inventory:catalog_write and grant it to tenant_owner.
-- 2) Recreate save_ingredient_catalog with a capability-based gate
--    (permission OR central_supply_ops position) instead of the
--    hardcoded owner role.
-- 3) Restore the per-ingredient unit cap of 20 that the
--    20260820030125 recreation regressed back to 3.

-- ── Permission catalog ──

INSERT INTO public.permission_keys (key, module, description, scope, is_delegable_to_staff)
VALUES
  ('inventory:catalog_write', 'inventory', 'Thêm/sửa nguyên liệu và thang đơn vị quy đổi', 'tenant', false)
...
WHERE key = 'inventory:catalog_write'
ON CONFLICT DO NOTHING;

-- ── Catalog RPC with capability-based authorization ──
...
COMMENT ON FUNCTION public.save_ingredient_catalog(
  bigint, text, text, bigint, text, text, numeric, numeric, numeric, integer, jsonb, text, bigint, bigint, bigint, boolean, boolean
) IS 'ADR 0045: atomic ingredient catalog save; capability-gated (inventory:catalog_write or central_supply_ops position); up to 20 units per ingredient.';
