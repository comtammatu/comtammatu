-- POS side quantities.
-- Allows one cart line such as "Com suon cot let + 2 Op la".
-- The side quantity is per main item unit. If the main line quantity is 2
-- and side quantity is 2, the order represents 2 main items with 2 sides each.
...
AS $$
  WITH side_input AS (
    ...
    FROM jsonb_array_elements(
      CASE
        WHEN p_sides IS NOT NULL AND jsonb_typeof(p_sides) = 'array'
          THEN p_sides
        ELSE '[]'::JSONB
      END
    ) AS side_el
...
BEGIN
  ...
  RETURN jsonb_build_object('success', true, 'order_id', p_order_id, 'added_count', jsonb_array_length(p_items));
END;
$$;
...
COMMENT ON COLUMN public.order_items.sides IS
  'Snapshot: [{"side_item_id": bigint, "name": text, "price": numeric, "quantity": int, "is_default": boolean}]';
