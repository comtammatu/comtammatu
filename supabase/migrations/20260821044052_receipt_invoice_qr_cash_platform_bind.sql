-- Cash and platform (delivery prepaid) confirm enqueue the receipt before the
-- HĐĐT issue job exists. attach_invoice_buyer_qr_* only stamps invoice_qr when
-- a queued draft job or not_required invoice is already present — so those
-- slips printed without "QUÉT QR XUẤT HĐĐT". VietQR works because
-- invoiceSnapshot is on the payment before completion.
--
-- After binding the snapshot / issue job, re-touch the receipt print_jobs.payload
-- so the existing BEFORE UPDATE OF payload triggers attach the buyer QR.
...
BEGIN
  ...
  IF v_print_job_id IS NOT NULL THEN
    UPDATE public.print_jobs
       SET payload = COALESCE(payload, '{}'::jsonb) - 'invoice_qr'
...
COMMENT ON FUNCTION public.confirm_cash_payment_with_invoice_binding(bigint, numeric, jsonb) IS
  'Completes cash payment, queues HĐĐT, then re-touches the receipt print job so buyer invoice QR attaches.';

COMMENT ON FUNCTION public.confirm_platform_payment_with_invoice_binding(bigint, jsonb) IS
  'Completes platform prepaid, queues HĐĐT, then re-touches the receipt print job so buyer invoice QR attaches.';
