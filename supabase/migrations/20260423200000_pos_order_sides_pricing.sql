-- =============================================================
-- POS order sides pricing
-- Issue: order_items.sides JSONB only stored {side_item_id,name,is_default}
-- and create_order / append_order_items RPCs computed
--   v_unit_price := base + variant_adj + modifier_sum
-- completely ignoring sides. Customers selecting "Bì, Chả, Ốp la"
-- got charged zero for the sides, and printed bills/kitchen
-- tickets could not render side prices.
--
-- Fix:
--  1. RPCs look up menu_items.base_price for each side_item_id
--     and add v_sides_sum to v_unit_price.
--  2. RPCs persist enriched sides JSONB with price field so
--     receipt/KDS rendering can show "Kèm: Bì (15.000đ)".
--
-- All other RPC logic is identical to 20260425020000.
-- =============================================================
...
BEGIN
  ...
  LOOP
    ...
    -- Sides: lookup base_price of each side_item, sum it into unit price,
    -- and rebuild the sides JSONB with authoritative name + price.
    v_sides_sum := 0;
    v_enriched_sides := '[]'::jsonb;
    IF v_item -> 'sides' IS NOT NULL
       AND jsonb_typeof(v_item -> 'sides') = 'array'
       AND jsonb_array_length(v_item -> 'sides') > 0
    THEN
      SELECT
        COALESCE(SUM(mi.base_price), 0),
        COALESCE(
          jsonb_agg(
            jsonb_build_object(
              'side_item_id', mi.id,
              'name',         mi.name,
              'price',        mi.base_price,
              'is_default',   COALESCE((side_el ->> 'is_default')::boolean, false)
            )
          ),
          '[]'::jsonb
        )
      INTO v_sides_sum, v_enriched_sides
      FROM jsonb_array_elements(v_item -> 'sides') AS side_el
      JOIN public.menu_items mi
        ON mi.id = (side_el ->> 'side_item_id')::BIGINT
...
-- Update column comment to reflect new schema
COMMENT ON COLUMN public.order_items.sides IS
  'Snapshot: [{"side_item_id": bigint, "name": text, "price": numeric, "is_default": boolean}]';
