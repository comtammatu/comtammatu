CREATE TABLE public.bank_transaction_expense_matches (
  ...
  CONSTRAINT bank_transaction_expense_matches_event_expense_key
    UNIQUE (tenant_id, webhook_event_id, expense_id),
  CONSTRAINT bank_transaction_expense_matches_expense_key
    UNIQUE (tenant_id, expense_id)
);

CREATE INDEX bank_transaction_expense_matches_event_idx
  ON public.bank_transaction_expense_matches (tenant_id, webhook_event_id);

CREATE INDEX bank_transaction_expense_matches_expense_idx
  ON public.bank_transaction_expense_matches (tenant_id, expense_id);

ALTER TABLE public.bank_transaction_expense_matches ENABLE ROW LEVEL SECURITY;

CREATE POLICY bank_transaction_expense_matches_select
...
GRANT SELECT ON TABLE public.bank_transaction_expense_matches TO authenticated;
GRANT ALL ON TABLE public.bank_transaction_expense_matches TO service_role;
GRANT ALL ON SEQUENCE public.bank_transaction_expense_matches_id_seq TO service_role;
