-- Allow ingredient unit ladder edits after stock movements.
-- Replace permanent locks with a controlled base-unit rebase of current
-- stock, thresholds, WAC, and valuation quantities. Historical document and
-- movement snapshots stay immutable.
...
BEGIN
  ...
  FROM public.ingredient_units AS unit_row
  JOIN public.units AS units
    ON units.id = unit_row.unit_id
  ...
  JOIN public.ingredient_units AS unit_row
    ON unit_row.tenant_id = v_tenant
   AND unit_row.ingredient_id = v_id
   AND unit_row.unit_id = item.entry_unit_id
  JOIN public.units AS units
    ON units.id = unit_row.unit_id
   AND units.tenant_id = unit_row.tenant_id
