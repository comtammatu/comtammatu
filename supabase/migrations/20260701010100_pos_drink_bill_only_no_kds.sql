...
BEGIN
  ...
  FOR v_item IN
    SELECT oi.id AS order_item_id, mi.category_id, mc.type AS category_type
    ...
    ORDER BY oi.id
  LOOP
    IF v_item.category_type = 'drink' THEN
      CONTINUE;
    END IF;
...
COMMENT ON FUNCTION public.route_order_to_kds(p_order_id bigint) IS
  'Routes non-drink KDS-visible categories to KDS tickets and queues printer-only kitchen slips at POS dispatch time. Drink categories stay bill-only.';
