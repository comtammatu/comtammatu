-- route_order_to_kds silently dropped any order item whose category had no KDS
-- station, no fallback (catch-all) station, AND no kitchen-ticket printer route:
-- the per-item loop neither ticketed nor CONTINUE'd it, and the printer-dispatch
-- block never saw it, so the item ended with `sent_to_kitchen_at IS NULL` and no
-- `kds_tickets` row — the kitchen got nothing, yet the caller reported success.
--
-- Make the routing fail loud: after the per-item loop and the printer-dispatch
-- block, detect items that were neither ticketed (no kds_tickets row) nor
-- printer-dispatched (sent_to_kitchen_at still NULL) and are not cancelled, then
-- RAISE. Because route_order_to_kds runs inside the single create_order /
-- append_order_items / sendToKitchen statement, the RAISE rolls the partial send
-- back cleanly instead of half-committing it. Signature, SECURITY context,
-- search_path, and all routing behavior are otherwise identical.
...
BEGIN
  ...
  -- Fail-loud guard. After both the KDS-ticket loop and the printer-dispatch
  -- block, any non-cancelled item that is still unsent (sent_to_kitchen_at IS
  -- NULL) AND has no kds_tickets row was routed nowhere — its category has no
  -- active station, no fallback station, and no kitchen-ticket printer. Raising
  -- here rolls the whole send back so the order is never half-sent silently.
