...
-- Payment-method breakdowns expose cash, VietQR, and MoMo explicitly.
...
payment_daily AS (
  SELECT
    ...
    COALESCE(SUM(pp.payment_amount) FILTER (WHERE pp.payment_method = 'momo'), 0) AS momo_revenue
...
BEGIN
  ...
  payment_daily AS (
    SELECT
      ...
      COALESCE(SUM(pp.payment_amount) FILTER (WHERE pp.method = 'momo'), 0) AS momo_revenue
    FROM paid_payments pp
...
COMMENT ON FUNCTION public.get_revenue_kpis(BIGINT, DATE, DATE) IS
  'Finance KPI single row. net_revenue/cash/vietqr/momo are money collected from payments.amount; subtotal/discount/tax/order_count are distinct paid-order sales facts.';
...
COMMENT ON FUNCTION public.get_revenue_by_cashier(bigint, date, date) IS
  'Cashier revenue from completed cash, VietQR, and MoMo payments.';
