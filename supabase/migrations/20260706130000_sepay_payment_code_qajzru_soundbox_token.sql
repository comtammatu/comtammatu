-- Payment code fixed prefix moves to the new MB soundbox recognition token
-- 'QAJZRU5550 MBBMS01382716 1' plus the same 12-char CSPRNG suffix:
-- 'QAJZRU5550 MBBMS01382716 1 <12x [A-Z0-9]>'. The fixed prefix lets the MB
-- "loa thông báo" recognise the transfer; the random suffix stays the per-order
-- match key SePay reconciles on. Every space is a single space: the QR-memo
-- builder (print_vietqr_ascii) and the SePay webhook both collapse whitespace
-- runs to one space before matching, so the stored code must already be
-- single-spaced. Prior codes (fixed 'VQRLOAMB20260626100157757 …', dynamic
-- 'VQRLOAMB[0-9]{17}', and 'DH…') stay valid for in-flight orders.
...
CREATE OR REPLACE FUNCTION public.generate_order_payment_code() RETURNS text
    ...
    SET search_path TO ''
    AS $$
  -- 'QAJZRU5550 MBBMS01382716 1' is a fixed soundbox token, NOT a live
  -- timestamp; do not regenerate it. Uniqueness comes from the CSPRNG suffix.
  SELECT 'QAJZRU5550 MBBMS01382716 1 ' || string_agg(
    substr(
...
-- New format is the fixed literal token + 12-char suffix. The other three
-- branches are GRANDFATHER clauses only: they keep pre-existing codes valid so
-- updates to already-settled orders do not trip the check. The generator no
-- longer emits those shapes. NOT VALID + VALIDATE avoids an ACCESS EXCLUSIVE
-- rewrite; every existing row already matches a grandfather branch.
...
ALTER TABLE public.orders
  ...
  CHECK (payment_code ~* '^(QAJZRU5550 MBBMS01382716 1 [A-Z0-9]{12}|VQRLOAMB20260626100157757 [A-Z0-9]{12}|VQRLOAMB[0-9]{17}|DH[A-Z0-9]{3,12})$')
  NOT VALID;
...
-- is the fixed literal token + suffix; the three prior shapes stay as grandfather
-- branches so reprints of already-settled orders still render a QR. Reproduced
-- verbatim from the deployed definition except that one predicate.
...
BEGIN
  ...
  WHERE order_id = p_order_id
    ...
    AND status <> 'failed'
    AND provider_ref ~* '^(QAJZRU5550 MBBMS01382716 1 [A-Z0-9]{12}|VQRLOAMB20260626100157757 [A-Z0-9]{12}|VQRLOAMB[0-9]{17}|DH[A-Z0-9]{3,12})$'
  ORDER BY id DESC
  LIMIT 1;
