...
-- ############################################################################
-- V15.5 RE-ADDITIONS (KEEP-feature DB objects re-added after lean over-drop)
-- R1 item-discount (folded from forward migration 20260608090000) +
-- R1 lean enqueue print, R2 scheduling, R3 amend_grn_line, R4 notif unread.
-- ############################################################################

-- --- R1: item-level discount (columns + RPCs + total recompute) ---
...
-- (auth_tenant_id, has_permission, has_permission_any, log_audit,
-- compute_discount_amount, recompute_supplier_invoice_matching,
-- staff_role_from_position_code) already exists above.
--
-- R1 item-level discount columns + RPCs are folded in from the committed
-- forward migration 20260608090000 (appended just above this block). This
-- block adds: R1 lean enqueue print, R2 scheduling, R3 amend_grn_line,
-- R4 notifications unread.
-- ============================================================================


-- ----------------------------------------------------------------------------
-- R1 (print) — lean enqueue_edit_pending_order_item_quantity_print.
--
-- Routes a kitchen-visible quantity-delta print straight to the branch's
-- active kitchen printers (role kitchen_1 / kitchen_2), mirroring the V7
-- route_order_to_kds lean printer model. NO printer_menu_categories /
-- printer_print_types (cut in lean). Increase => kitchen_ticket (send_kind
-- append / GỌI THÊM); decrease => cancel_ticket. Payload uses the delta qty.
-- ----------------------------------------------------------------------------
...
BEGIN
  ...
  -- Lean routing: any active kitchen printer at the branch. kitchen_2 = slot 2.
  ...
  WHERE p.tenant_id = v_order.tenant_id
    ...
    AND p.is_active = TRUE
    AND p.role IN ('kitchen_1', 'kitchen_2')
  ORDER BY CASE WHEN p.role = 'kitchen_1' THEN 0 ELSE 1 END, p.id
  LIMIT 1;
  ...
  v_idempotency := 'order:' || v_order.id::TEXT
    ...
    || ':printer:' || v_route.printer_id::TEXT
    || ':at:' || v_event_token;
  ...
  INSERT INTO public.print_jobs (
    ...
    v_order.tenant_id, v_order.branch_id, v_route.printer_id, v_print_type,
...
COMMENT ON FUNCTION public.enqueue_edit_pending_order_item_quantity_print(p_order_item_id bigint, p_old_quantity integer, p_new_quantity integer, p_reason text) IS 'Lean kitchen-visible print delta after a sent pending item quantity edit. Increase => kitchen_ticket (send_kind=append / GỌI THÊM); decrease => cancel_ticket. Routes directly to active branch kitchen printers (role kitchen_1/kitchen_2); NO printer_menu_categories/printer_print_types. Returns skipped reasons not_sent, no_quantity_change, no_printer.';


-- ----------------------------------------------------------------------------
-- R2 — lean scheduling tables (shift_assignments, shift_requests) + workflow
-- RPCs (submit/cancel/approve/reject). shifts already exists in the baseline.
-- Original shape from _legacy, trimmed lean. leave_requests intentionally
-- omitted (clock/schedule/shift-register read shift_assignments only).
-- ----------------------------------------------------------------------------
...
ALTER TABLE public.shift_assignments ALTER COLUMN id ADD GENERATED ALWAYS AS IDENTITY (
...
-- One assignment per employee per day (matches approve_shift_request ON CONFLICT).
ALTER TABLE ONLY public.shift_assignments
    ADD CONSTRAINT shift_assignments_employee_date_uniq UNIQUE (employee_id, date, tenant_id);

ALTER TABLE ONLY public.shift_assignments
    ADD CONSTRAINT shift_assignments_tenant_id_fkey FOREIGN KEY (tenant_id) REFERENCES public.tenants(id) ON DELETE CASCADE;
ALTER TABLE ONLY public.shift_assignments
    ADD CONSTRAINT shift_assignments_branch_id_fkey FOREIGN KEY (branch_id) REFERENCES public.branches(id) ON DELETE CASCADE;
ALTER TABLE ONLY public.shift_assignments
...
CREATE INDEX idx_shift_assignments_branch_date ON public.shift_assignments USING btree (tenant_id, branch_id, date);
CREATE INDEX idx_shift_assignments_employee_date ON public.shift_assignments USING btree (tenant_id, employee_id, date);
CREATE INDEX idx_shift_assignments_shift ON public.shift_assignments USING btree (shift_id);

COMMENT ON TABLE public.shift_assignments IS 'Lean roster: one shift assignment per employee per day. Read by employee/schedule + clock shift-window guard; managed via shift-assignment-actions + approve_shift_request.';
...
CREATE TABLE public.shift_requests (
    ...
    shift_id bigint NOT NULL,
    date date NOT NULL,
    status public.shift_request_status DEFAULT 'pending'::public.shift_request_status NOT NULL,
    note text,
    reviewed_by uuid,
...
-- One pending request per employee/shift/day (matches shift-register 23505 path).
CREATE UNIQUE INDEX shift_requests_pending_uniq ON public.shift_requests
    USING btree (employee_id, shift_id, date, tenant_id)
    WHERE (status = 'pending'::public.shift_request_status);

ALTER TABLE ONLY public.shift_requests
    ADD CONSTRAINT shift_requests_tenant_id_fkey FOREIGN KEY (tenant_id) REFERENCES public.tenants(id) ON DELETE CASCADE;
ALTER TABLE ONLY public.shift_requests
    ADD CONSTRAINT shift_requests_branch_id_fkey FOREIGN KEY (branch_id) REFERENCES public.branches(id) ON DELETE CASCADE;
ALTER TABLE ONLY public.shift_requests
    ADD CONSTRAINT shift_requests_employee_id_fkey FOREIGN KEY (employee_id) REFERENCES public.employees(id) ON DELETE CASCADE;
ALTER TABLE ONLY public.shift_requests
    ADD CONSTRAINT shift_requests_shift_id_fkey FOREIGN KEY (shift_id) REFERENCES public.shifts(id) ON DELETE CASCADE;
ALTER TABLE ONLY public.shift_requests
    ADD CONSTRAINT shift_requests_reviewed_by_fkey FOREIGN KEY (reviewed_by) REFERENCES auth.users(id) ON DELETE SET NULL;
ALTER TABLE ONLY public.shift_requests
    ADD CONSTRAINT shift_requests_assignment_id_fkey FOREIGN KEY (assignment_id) REFERENCES public.shift_assignments(id) ON DELETE SET NULL;

CREATE INDEX idx_shift_requests_branch_status ON public.shift_requests USING btree (tenant_id, branch_id, status, date);
CREATE INDEX idx_shift_requests_employee ON public.shift_requests USING btree (tenant_id, employee_id, date);

COMMENT ON TABLE public.shift_requests IS 'Lean shift-registration workflow: staff submit a pending request, manager approves (creates a shift_assignment) or rejects. Driven by submit/cancel/approve/reject_shift_request RPCs.';

-- R2 RLS
ALTER TABLE public.shift_assignments ENABLE ROW LEVEL SECURITY;

CREATE POLICY shift_assignments_select ON public.shift_assignments FOR SELECT TO authenticated USING (((tenant_id = public.auth_tenant_id()) AND ((EXISTS ( SELECT 1
   FROM public.employees e
  WHERE ((e.id = shift_assignments.employee_id) AND (e.profile_id = auth.uid())))) OR public.has_permission_any('staff:view'::text) OR public.has_permission_any('hr:view_employee'::text))));

CREATE POLICY shift_assignments_write ON public.shift_assignments TO authenticated USING (((tenant_id = public.auth_tenant_id()) AND public.has_permission_any('staff:manage'::text))) WITH CHECK (((tenant_id = public.auth_tenant_id()) AND public.has_permission_any('staff:manage'::text)));

ALTER TABLE public.shift_requests ENABLE ROW LEVEL SECURITY;

CREATE POLICY shift_requests_select ON public.shift_requests FOR SELECT TO authenticated USING (((tenant_id = public.auth_tenant_id()) AND ((EXISTS ( SELECT 1
   FROM public.employees e
  WHERE ((e.id = shift_requests.employee_id) AND (e.profile_id = auth.uid())))) OR public.has_permission(branch_id, 'hr:approve_shift_request'::text) OR public.has_permission_any('hr:view_employee'::text))));

CREATE POLICY shift_requests_self_insert ON public.shift_requests FOR INSERT TO authenticated WITH CHECK (((tenant_id = public.auth_tenant_id()) AND (status = 'pending'::public.shift_request_status) AND (date >= CURRENT_DATE) AND (assignment_id IS NULL) AND (reviewed_by IS NULL) AND (reviewed_at IS NULL) AND public.has_permission_any('hr:register_shift'::text) AND (EXISTS ( SELECT 1
   FROM public.employees e
  WHERE ((e.id = shift_requests.employee_id) AND (e.profile_id = auth.uid()) AND (e.tenant_id = public.auth_tenant_id()) AND (e.is_active = true))))));

CREATE POLICY shift_requests_manager_update ON public.shift_requests FOR UPDATE TO authenticated USING (((tenant_id = public.auth_tenant_id()) AND public.has_permission(branch_id, 'hr:approve_shift_request'::text))) WITH CHECK (((tenant_id = public.auth_tenant_id()) AND public.has_permission(branch_id, 'hr:approve_shift_request'::text)));
...
-- R2 workflow RPCs (lean, from _legacy shape).
CREATE OR REPLACE FUNCTION public.submit_shift_request(p_branch_id bigint, p_shift_id bigint, p_date date, p_note text DEFAULT NULL::text) RETURNS bigint
    LANGUAGE plpgsql SECURITY DEFINER
    SET search_path TO 'public', 'pg_temp'
    AS $$
DECLARE
...
CREATE OR REPLACE FUNCTION public.cancel_shift_request(p_request_id bigint) RETURNS void
    LANGUAGE plpgsql SECURITY DEFINER
    SET search_path TO 'public', 'pg_temp'
    AS $$
DECLARE
...
BEGIN
  ...
  SET status = 'cancelled', reviewed_by = v_user_id, reviewed_at = now(), updated_at = now()
...
CREATE OR REPLACE FUNCTION public.approve_shift_request(p_request_id bigint) RETURNS bigint
    LANGUAGE plpgsql SECURITY DEFINER
    SET search_path TO 'public', 'pg_temp'
    AS $$
DECLARE
...
BEGIN
  ...
  END IF;
  UPDATE public.shift_requests
  SET status = 'approved', reviewed_by = v_user_id, reviewed_at = now(), assignment_id = v_assignment_id, updated_at = now()
  WHERE id = p_request_id;
  PERFORM public.log_audit(
...
CREATE OR REPLACE FUNCTION public.reject_shift_request(p_request_id bigint, p_reason text DEFAULT NULL::text) RETURNS void
    LANGUAGE plpgsql SECURITY DEFINER
    SET search_path TO 'public', 'pg_temp'
    AS $$
DECLARE
...
BEGIN
  ...
  END IF;
  UPDATE public.shift_requests
  SET status = 'rejected', reviewed_by = v_user_id, reviewed_at = now(), rejected_reason = p_reason, updated_at = now()
  WHERE id = p_request_id;
  PERFORM public.log_audit(
...
-- ----------------------------------------------------------------------------
-- R3 — amend_grn_line (lean). Amend a confirmed GRN line qty/cost; emit a
-- compensating grn_amend stock_movement at branch grain (trigger
-- trg_update_stock_on_movement folds it into stock_levels.current_quantity),
-- consistent with the V12 confirm_goods_receipt_note. NO supplier_price_list,
-- NO inventory_locations, NO purchase_orders, NO supplier_returns. Still
-- recomputes any linked supplier_invoices (kept in lean).
-- ----------------------------------------------------------------------------
CREATE OR REPLACE FUNCTION public.amend_grn_line(p_grn_id bigint, p_line_id bigint, p_received_quantity numeric, p_unit_cost numeric, p_reason text, p_rejected_quantity numeric DEFAULT NULL::numeric) RETURNS jsonb
    LANGUAGE plpgsql SECURITY DEFINER
    SET search_path TO 'public'
    AS $$
DECLARE
...
COMMENT ON FUNCTION public.amend_grn_line(p_grn_id bigint, p_line_id bigint, p_received_quantity numeric, p_unit_cost numeric, p_reason text, p_rejected_quantity numeric) IS 'Lean post-confirm GRN line amendment. NET delta = (received - rejected). Compensating grn_amend stock_movement -> trg_update_stock_on_movement updates stock_levels.current_quantity at branch grain. Recomputes linked supplier_invoices. NO supplier_price_list / inventory_locations / purchase_orders / supplier_returns (cut in lean). Permission: procurement:grn_amend.';

GRANT EXECUTE ON FUNCTION public.amend_grn_line(bigint, bigint, numeric, numeric, text, numeric) TO authenticated;


-- ----------------------------------------------------------------------------
-- R4 — notifications unread. Lean: a single notifications.read_at column
-- replaces the dropped per-user notification_reads junction. count/mark RPCs
-- fold over visible (role+branch-scoped) rows with read_at IS NULL.
-- ----------------------------------------------------------------------------
ALTER TABLE public.notifications
  ADD COLUMN IF NOT EXISTS read_at timestamp with time zone;

COMMENT ON COLUMN public.notifications.read_at IS 'Lean read marker (replaces the dropped per-user notification_reads junction). NULL = unread; set to now() by mark_all_notifications_read.';

CREATE OR REPLACE FUNCTION public.count_unread_notifications() RETURNS bigint
    LANGUAGE sql STABLE SECURITY DEFINER
    SET search_path TO ''
    AS $$
...
COMMENT ON FUNCTION public.count_unread_notifications() IS 'Lean unread count: visible (role+branch-scoped) notifications with read_at IS NULL. No notification_reads junction.';
...
COMMENT ON FUNCTION public.mark_all_notifications_read() IS 'Lean mark-all: set read_at=now() on visible (role+branch-scoped) unread notifications. Returns rows affected. No notification_reads junction.';

GRANT EXECUTE ON FUNCTION public.count_unread_notifications() TO authenticated;
GRANT EXECUTE ON FUNCTION public.mark_all_notifications_read() TO authenticated;

-- ============================================================================
-- END V15.5 RE-ADDITIONS
-- ============================================================================
