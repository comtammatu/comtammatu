...
BEGIN
  ...
  IF NOT FOUND THEN
    RAISE EXCEPTION 'sepay_replay_payment_not_pending' USING ERRCODE = 'P0002';
  END IF;

  IF lower(COALESCE(v_payment.payment_code, '')) <> lower(v_payment_code)
    OR lower(COALESCE(v_payment.provider_ref, '')) <> lower(v_payment_code)
  THEN
  ...
  SELECT public.reconcile_sepay_order_evidence(v_event.id, v_payment_code)
  INTO v_result;
...
COMMENT ON FUNCTION public.replay_signed_sepay_payment_evidence(
  ...
  text,
  uuid
) IS 'Replays one signed incoming SePay event against its exact pending VietQR payment. The service caller supplies an authenticated Owner actor, which is revalidated and audited in the same transaction.';
