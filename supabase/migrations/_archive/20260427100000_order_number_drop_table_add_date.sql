-- =========================================================================
-- Order number format: drop table_number, add YYMMDD
--
-- Problem:
--   Current format `TC-BAN{table}-{seq:03}` / `TC-{seq:03}` / `MV-{seq:03}`
--   has two issues:
--   1. Daily counter resets seq to 1, but order_number UNIQUE is NOT
--      scoped by date → day-2's `TC-BAN5-001` collides with day-1's row.
--      Creating the second day's first dine-in throws unique_violation.
--   2. `BAN{table}` becomes stale after transfer/merge table — an order
--      started at bàn 5 keeps showing `TC-BAN5-001` even after moving
--      to bàn 8. Table metadata belongs on orders.table_id, not baked
--      into an immutable identifier.
--
-- Fix:
--   New format — date replaces table, table removed from identifier:
--     Dine-in:  TC-{YYMMDD}-{seq:03}   e.g. TC-260427-001
--     Takeaway: MV-{YYMMDD}-{seq:03}   e.g. MV-260427-001
--
--   - Counter scope unchanged: (tenant, branch, date, order_type)
--   - UNIQUE constraint on orders unchanged — date in the string now
--     makes cross-day collisions impossible
--   - Transfer/merge table: order_number bất biến (table shown via
--     orders.table_id → tables.number live lookup)
--
-- Existing rows with the old format keep their identifiers — UNIQUE
-- allows both formats to coexist. No data migration.
-- =========================================================================
...
BEGIN
  ...
  -- NEW format — YYMMDD replaces table_number. `TC-BAN{n}` dropped so
  -- that transfer_table / merge_order don't leave stale identifiers.
  v_date_part := to_char(
