...
--   check_out directly.
-- - Branch Manager approves branch-staff requests for their own branch.
-- - Branch Manager checkout escalates to owner/super_manager.
...
COMMENT ON COLUMN public.attendance_records.checkout_approval_target_roles IS
  'Access buckets that should approve this checkout request. Branch staff target branch_manager; manager and non-branch staff target owner/super_manager.';
...
SET
  checkout_requested_by_role = requester.requester_role,
  checkout_approval_target_roles = CASE
    WHEN requester.requester_role = 'branch_manager'
      THEN ARRAY['owner', 'super_manager']::text[]
    WHEN requester.requester_role IN ('cashier', 'waiter', 'chef')
...
CROSS JOIN LATERAL (
  SELECT COALESCE(private.staff_role_from_position_code(po.code), 'office') AS requester_role
) requester
...
DROP FUNCTION IF EXISTS public.employee_request_clock_out_with_code(
  bigint,
  bigint,
  bigint
);
...
BEGIN
  ...
  IF v_requester_role = 'branch_manager' THEN
    IF v_approver_role NOT IN ('owner', 'super_manager') THEN
  ...
  ELSIF v_approver_role = 'branch_manager' THEN
    ...
    END IF;
  ELSIF v_approver_role NOT IN ('owner', 'super_manager') THEN
