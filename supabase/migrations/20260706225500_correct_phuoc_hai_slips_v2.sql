-- Migration: Correct Phước Hải branch's 2 wrongly approved count slips from Kho CN to Bếp CN (v2 with correct branch name and robust inline unit conversion).
-- Created At: 2026-07-06T22:55:00Z
...
BEGIN
  ...
  -- Matches both seed 'Chi nhánh Phước Hải' and production 'Phước Hải'
  FOR v_slip IN 
    ...
    WHERE b.name IN ('Phước Hải', 'Chi nhánh Phước Hải')
  ...
  LOOP
    ...
    IF v_kitchen_id IS NOT NULL THEN
      ...
      LOOP
        ...
        -- Convert counted quantity to base unit using robust inline lookup
        IF v_line.entry_unit_id IS NULL THEN
          v_counted_base := v_line.counted_quantity;
        ELSE
          SELECT to_base_factor INTO v_factor
          FROM public.ingredient_units
          WHERE ingredient_id = v_line.ingredient_id
            AND unit_id = v_line.entry_unit_id
            AND tenant_id = v_slip.tenant_id
            AND is_active;

          IF v_factor IS NULL THEN
            IF v_line.counted_quantity = 0 THEN
              v_factor := 1.000; -- Safe fallback for zero quantity with missing conversion config
            ELSE
              RAISE EXCEPTION 'Unit % is not active/defined for ingredient % under tenant %', 
                v_line.entry_unit_id, v_line.ingredient_id, v_slip.tenant_id;
            END IF;
          END IF;
          v_counted_base := v_line.counted_quantity * v_factor;
        END IF;
