-- =============================================================
-- POS/KDS: shared TC/MV order sequence + kitchen ticket batches (v2)
--
-- Supersedes 20260430190023_pos_kitchen_ticket_sequence.sql which was
-- never applied to production. v1 redefined split_order with the legacy
-- BIGINT[] signature, conflicting with the JSONB partial-quantity overload
-- already live on remote (introduced out-of-order by 20260428021437).
-- v2 keeps split_order at JSONB(p_item_partials) and only updates its
-- counter-mint block to use the new shared (tenant,branch,date) sequence.
--
...
-- 5. split_order(JSONB) — body unchanged from 20260428021437 except the
--    counter-mint block now uses the shared (tenant,branch,date) sequence
--    (no more order_type column on order_daily_counters).
...
BEGIN
  ...
  -- TC/MV is a prefix-only convention now; the sequence pool is shared.
  INSERT INTO public.order_daily_counters (
    tenant_id, branch_id, counter_date, last_seq
  )
  VALUES (
