-- Migration: make POS/menu-limit stock availability reserve shared recipe ingredients,
-- not each sale menu item in isolation.
...
AS $$
  ...
  SELECT
    ...
    CASE
      ...
      WHEN c.stock_remaining_effective IS NULL AND c.manual_remaining IS NULL THEN NULL
      WHEN c.stock_remaining_effective IS NULL THEN GREATEST(0, c.manual_remaining)
      WHEN c.manual_remaining IS NULL THEN GREATEST(0, c.stock_remaining_effective)
      ELSE GREATEST(0, LEAST(c.stock_remaining_effective, c.manual_remaining))
...
COMMENT ON FUNCTION public.branch_menu_limit_availability(
  p_tenant_id bigint,
  p_branch_id bigint,
  p_limit_date date,
  p_stock_gate_enabled boolean,
  p_exclude_hold_tokens uuid[]
) IS 'Stock availability reserves shared recipe ingredients across all menu items using the same ingredient pool; manual daily caps still apply per menu item. Capacity NULL remains unlimited per D064 fail-open doctrine.';
