...
BEGIN
  ...
  v_hddt_total := GREATEST(
    0,
    COALESCE(v_order.total_amount, 0) - COALESCE(v_order.service_charge, 0)
  );
  ...
  ELSE
    ...
    IF jsonb_typeof(v_payload -> 'draftSnapshot') IS DISTINCT FROM 'object' THEN
      ...
      v_payload := v_buyer_payload || jsonb_build_object(
        ...
        jsonb_build_object(
          ...
          'subtotal', v_hddt_total,
          'vatRate', 0,
          'vatAmount', 0,
          'totalAmount', v_hddt_total,
          'items', v_items
        )
      );
    ...
    ELSE
      v_payload := jsonb_set(
        jsonb_set(
          ...
          to_jsonb(v_hddt_total),
          false
        ),
        '{draftSnapshot,totalAmount}',
        to_jsonb(v_hddt_total),
        false
      );
    END IF;
    ...
    ELSE
      ...
      SET subtotal = v_hddt_total,
          total_amount = v_hddt_total,
...
ALTER TABLE public.tax_invoice_issue_jobs
  DISABLE TRIGGER tax_invoice_issue_jobs_snapshot_before_write;
...
WITH canonical AS (
  SELECT
    ...
    GREATEST(
      ...
      COALESCE(order_row.total_amount, 0)
        - COALESCE(order_row.service_charge, 0)
    ) AS hddt_total
  FROM public.tax_invoice_issue_jobs job
  JOIN public.orders order_row
    ON order_row.id = job.order_id
   AND order_row.tenant_id = job.tenant_id
   AND order_row.branch_id = job.branch_id
  ...
  WHERE job.status IN ('queued', 'blocked')
    AND jsonb_typeof(job.invoice_payload -> 'draftSnapshot') = 'object'
)
UPDATE public.tax_invoice_issue_jobs job
SET invoice_payload = jsonb_set(
      jsonb_set(
        job.invoice_payload,
        '{draftSnapshot,subtotal}',
        to_jsonb(canonical.hddt_total),
        false
      ),
      '{draftSnapshot,totalAmount}',
      to_jsonb(canonical.hddt_total),
    ...
    updated_at = now()
FROM canonical
WHERE job.id = canonical.job_id;

ALTER TABLE public.tax_invoice_issue_jobs
  ENABLE TRIGGER tax_invoice_issue_jobs_snapshot_before_write;

UPDATE public.tax_invoices invoice
...
WHERE invoice.id = job.tax_invoice_id
  AND invoice.status = 'draft'
  AND invoice.invoice_number IS NULL
  AND job.status IN ('queued', 'blocked');
