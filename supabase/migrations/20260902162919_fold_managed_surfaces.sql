...
DECLARE
  ...
  v_before text;
BEGIN
  -- The live baseline already ships the two-arg VAT-aware function. The
  -- historical one-arg overload exists only on pre-baseline environments.
  IF to_regprocedure('public.record_bank_transaction_cash_deposit(bigint)') IS NULL THEN
    RETURN;
  END IF;
...
SELECT cron.schedule('inventory-valuation-reconciliation-daily', '15 23 * * *', 'SELECT public.run_inventory_valuation_reconciliation();');
SELECT cron.schedule('scan-order-delay-sla',               '* * * * *',  'SELECT public.scan_order_delay_sla();');
