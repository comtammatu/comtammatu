-- Keep the POS "Phuc vu" action consistent across the order header and item
-- rows. Marking an order as served means the active items have been served;
-- payment close remains separate and still must not force item/KDS statuses.
...
COMMENT ON FUNCTION public.update_pos_order_status(BIGINT, TEXT) IS
  'POS order status update. Marking served updates active order_items and KDS tickets to served; payment close remains handled by finalize_paid_order.';
