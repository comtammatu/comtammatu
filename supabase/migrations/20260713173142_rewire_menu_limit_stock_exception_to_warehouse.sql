...
BEGIN
  ...
  LEFT JOIN LATERAL (
    ...
    ORDER BY
      loc.is_default_consumption DESC,
      loc.is_default_issue DESC,
      loc.is_default_receive DESC,
      loc.sort_order NULLS LAST,
      loc.id
    LIMIT 1
  ) il ON TRUE
  ...
  IF v_branch_kind IS DISTINCT FROM 'branch' THEN
    RAISE EXCEPTION 'branch_warehouse_required' USING ERRCODE = '22023';
  END IF;

  IF v_location_id IS NULL THEN
    RAISE EXCEPTION 'default_warehouse_location_required' USING ERRCODE = '23502';
  END IF;
  ...
  LOOP
    INSERT INTO public.stock_movements (
      ...
      'Menu-Limits warehouse replenishment +' || p_extra_portions::text || ': ' || v_reason,
      v_uid,
      v_location_id,
...
REVOKE ALL ON FUNCTION public.add_menu_item_kitchen_stock_exception(bigint, bigint, integer, text)
  FROM PUBLIC, anon, authenticated;
GRANT EXECUTE ON FUNCTION public.add_menu_item_kitchen_stock_exception(bigint, bigint, integer, text)
  TO authenticated;
GRANT EXECUTE ON FUNCTION public.add_menu_item_kitchen_stock_exception(bigint, bigint, integer, text)
  TO service_role;

COMMENT ON FUNCTION public.add_menu_item_kitchen_stock_exception(bigint, bigint, integer, text)
IS 'Menu-Limits controlled +1/+2 branch warehouse stock adjustment by menu item recipe.';
