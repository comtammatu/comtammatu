-- create_payment trusted its caller-supplied identity parameters.
--
-- The RPC is SECURITY DEFINER and GRANTed to authenticated, but it took
-- p_tenant_id / p_branch_id / p_created_by and used them verbatim with no
-- authorization check. Any authenticated user could call it directly with
-- spoofed parameters to create/complete a payment for an order in any branch,
-- mark that order 'paid', and attribute the payment to another user.
--
-- Fix: in-function authorization, mirroring confirm_cash_payment — verify the
-- tenant and actor against the auth context (not the params) and require the
-- branch-scoped pos:confirm_payment permission. The order lookup already ties
-- the order to p_branch_id, so the branch-scoped check blocks cross-branch use.
-- The happy path is unchanged: the POS client passes the session tenant, the
-- session user id, and the route branch, all of which satisfy the checks.

CREATE OR REPLACE FUNCTION public.create_payment(
  p_tenant_id bigint, p_branch_id bigint, p_order_id bigint, p_method text,
  p_amount numeric, p_created_by uuid, p_provider_ref text DEFAULT NULL::text,
...
BEGIN
  ...
  -- In-function authorization: tenant / branch / actor are caller-supplied, so
  -- verify them against the auth context instead of trusting the parameters.
