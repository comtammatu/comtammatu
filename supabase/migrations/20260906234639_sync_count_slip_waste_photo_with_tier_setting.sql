-- Migration: sync_count_slip_waste_photo_with_tier_setting
-- Sync count slip waste photo validation with inventory_waste_tier_enabled setting.
-- When waste tiering is disabled (inventory_waste_tier_enabled = false), count slip approval
-- does not require photos for shortage lines.
...
BEGIN
  ...
  -- Load tier_enabled: check branch_settings first, then system_settings
  SELECT value INTO v_setting_val
  FROM public.branch_settings
  WHERE branch_id = v_slip.branch_id
    AND tenant_id = v_tenant
  ...
  -- Validate photo requirements only when waste tiering is enabled
  -- and only for physical waste reasons (spoiled, expired, damaged)
  IF v_tier_enabled AND EXISTS (
    SELECT 1
...
COMMENT ON FUNCTION public.approve_inventory_count_slip_with_waste(p_slip_id bigint, p_photo_urls jsonb, p_reasons jsonb, p_allow_self_review boolean, p_adjust_surplus boolean, p_surplus_reasons jsonb) IS 'Atomically approves a count slip, creates confirmed shortage writeoff and optional surplus count_adjustment movements with reasons and audited self-review.';

REVOKE ALL ON FUNCTION public.approve_inventory_count_slip_with_waste(bigint, jsonb, jsonb, boolean, boolean, jsonb) FROM PUBLIC;
GRANT ALL ON FUNCTION public.approve_inventory_count_slip_with_waste(bigint, jsonb, jsonb, boolean, boolean, jsonb) TO authenticated;
GRANT ALL ON FUNCTION public.approve_inventory_count_slip_with_waste(bigint, jsonb, jsonb, boolean, boolean, jsonb) TO service_role;
