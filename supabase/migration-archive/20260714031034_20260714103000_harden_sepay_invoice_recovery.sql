-- Keep per-order HĐĐT issuance and daily B2C summary aggregation mutually
-- exclusive before either flow calls the external invoice provider.
...
CREATE OR REPLACE FUNCTION public.hddt_assert_per_order_invoice_slot()
RETURNS trigger
LANGUAGE plpgsql
SECURITY DEFINER
SET search_path TO ''
...
BEGIN
  IF NEW.order_id IS NULL
     OR NEW.status IN ('cancelled', 'replaced', 'not_required') THEN
    RETURN NEW;
  END IF;

  PERFORM pg_advisory_xact_lock(
    hashtextextended('hddt-order:' || NEW.order_id::text, 0)
  );

  IF EXISTS (
    SELECT 1
    FROM public.tax_invoice_orders tio
    JOIN public.tax_invoices summary_invoice
      ON summary_invoice.id = tio.tax_invoice_id
    WHERE tio.tenant_id = NEW.tenant_id
      AND tio.order_id = NEW.order_id
      AND summary_invoice.status NOT IN ('cancelled', 'replaced')
  ) THEN
    RAISE EXCEPTION 'order % already in active daily summary', NEW.order_id
      USING ERRCODE = '23505';
  END IF;
...
BEGIN
  ...
  IF EXISTS (
    SELECT 1
    FROM public.tax_invoices direct_invoice
    WHERE direct_invoice.tenant_id = NEW.tenant_id
      AND direct_invoice.order_id = NEW.order_id
      AND direct_invoice.status NOT IN (
        'cancelled',
        'replaced',
        'not_required'
      )
  ) THEN
    RAISE EXCEPTION 'order % already has active per-order invoice', NEW.order_id
      USING ERRCODE = '23505';
  END IF;
...
DROP TRIGGER IF EXISTS trg_tax_invoices_assert_per_order_slot
  ON public.tax_invoices;
CREATE TRIGGER trg_tax_invoices_assert_per_order_slot
BEFORE INSERT ON public.tax_invoices
FOR EACH ROW
EXECUTE FUNCTION public.hddt_assert_per_order_invoice_slot();

REVOKE ALL ON FUNCTION public.hddt_assert_per_order_invoice_slot()
  FROM PUBLIC, anon, authenticated;
REVOKE ALL ON FUNCTION public.tio_assert_one_active_summary_per_order()
  FROM PUBLIC, anon, authenticated;
GRANT EXECUTE ON FUNCTION public.hddt_assert_per_order_invoice_slot()
  TO service_role;
GRANT EXECUTE ON FUNCTION public.tio_assert_one_active_summary_per_order()
  TO service_role;
