...
ALTER TABLE public.goods_received_notes
  ADD COLUMN IF NOT EXISTS location_id bigint REFERENCES public.inventory_locations(id) ON DELETE RESTRICT;

UPDATE public.goods_received_notes g
SET location_id = (
  SELECT il.id
  FROM public.inventory_locations il
  WHERE il.tenant_id = g.tenant_id
    AND il.branch_id = g.branch_id
...
CREATE INDEX IF NOT EXISTS idx_goods_received_notes_location
  ON public.goods_received_notes(tenant_id, location_id)
...
COMMENT ON FUNCTION public.confirm_goods_receipt_note(p_grn_id bigint) IS 'Atomic confirm GRN. Stock += (received_quantity - rejected_quantity) at the selected receipt location. PO-less GRNs create a received PO for 3-way matching. Permission: procurement:grn_confirm.';
