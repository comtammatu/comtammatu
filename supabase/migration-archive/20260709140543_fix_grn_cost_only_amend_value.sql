...
BEGIN
  ...
  IF abs(v_delta_base) > c_numeric_15_3_max
     OR abs(v_delta_qty) > c_numeric_15_3_max
     OR abs(v_delta_value) > c_numeric_15_2_max THEN
    RAISE EXCEPTION 'invalid_amount' USING ERRCODE = '22023';
  END IF;
  ...
  IF (v_delta_base <> 0 OR v_delta_value <> 0)
     AND v_current_qty + v_delta_base > 0 THEN
    v_next_wac := ROUND(
      ((v_current_qty * COALESCE(v_current_wac, 0)) + v_delta_value)
  ...
  PERFORM public.log_audit(
    ...
    jsonb_build_object(
      'line_id', p_line_id,
      'received_quantity', v_old_qty,
      'rejected_quantity', v_old_rej,
      'unit_cost', v_old_cost,
      'net_quantity', v_old_net,
      'net_base_quantity', v_old_net_base,
      'total_cost', ROUND(v_old_qty * v_old_cost, 2)
    ),
    jsonb_build_object(
      'line_id', p_line_id,
      'received_quantity', v_new_qty,
      'rejected_quantity', v_new_rej,
      'unit_cost', v_new_cost,
      'net_quantity', v_new_net,
      'net_base_quantity', v_new_net_base,
      'total_cost', v_new_total_cost,
      'delta_quantity', v_delta_qty,
...
COMMENT ON FUNCTION public.amend_grn_line(
  ...
  text,
  numeric
) IS 'Owner-only post-confirm GRN line amendment. Delta posts to the GRN receipt location, revalues current WAC by net value delta, and writes audit history on the GRN header.';
...
WITH missed_value AS (
  SELECT
    al.tenant_id,
    g.branch_id,
    gi.ingredient_id,
    (al.new_data->>'location_id')::bigint AS location_id,
    SUM(
      (al.new_data->>'delta_value')::numeric
      - CASE
          WHEN (al.new_data->>'delta_base_quantity')::numeric > 0 THEN
            (al.new_data->>'delta_base_quantity')::numeric
            * CASE
                WHEN (al.new_data->>'net_base_quantity')::numeric <> 0 THEN
                  ROUND(
                    ((al.new_data->>'net_quantity')::numeric * (al.new_data->>'unit_cost')::numeric)
                    / (al.new_data->>'net_base_quantity')::numeric,
                    2
                  )
                ELSE (al.new_data->>'unit_cost')::numeric
              END
          WHEN (al.new_data->>'delta_base_quantity')::numeric < 0 THEN
            (al.new_data->>'delta_base_quantity')::numeric
            * CASE
                WHEN (al.old_data->>'net_base_quantity')::numeric <> 0 THEN
                  ROUND(
                    ((al.old_data->>'net_quantity')::numeric * (al.old_data->>'unit_cost')::numeric)
                    / (al.old_data->>'net_base_quantity')::numeric,
                    2
                  )
                ELSE (al.old_data->>'unit_cost')::numeric
              END
          ELSE 0
        END
    ) AS missing_value
  ...
  JOIN public.goods_received_notes g
    ON g.id = al.entity_id
   AND g.tenant_id = al.tenant_id
  JOIN public.grn_items gi
    ON gi.id = (al.new_data->>'line_id')::bigint
   AND gi.grn_id = g.id
   AND gi.tenant_id = al.tenant_id
  WHERE al.action = 'inventory.grn.line_amended'
    AND al.entity_type = 'goods_received_note'
    AND al.new_data ? 'delta_value'
    AND al.new_data ? 'delta_base_quantity'
    AND al.new_data ? 'net_base_quantity'
    AND al.new_data ? 'net_quantity'
    AND al.new_data ? 'unit_cost'
    AND al.new_data ? 'location_id'
    AND al.old_data ? 'net_base_quantity'
    AND al.old_data ? 'net_quantity'
    AND al.old_data ? 'unit_cost'
  GROUP BY al.tenant_id, g.branch_id, gi.ingredient_id, (al.new_data->>'location_id')::bigint
),
repair AS (
  SELECT
    sl.id,
    ROUND(
      ((sl.current_quantity * COALESCE(sl.avg_unit_cost, 0)) + mv.missing_value)
      / sl.current_quantity,
      2
    ) AS next_wac
  FROM public.stock_levels sl
  JOIN missed_value mv
    ON mv.tenant_id = sl.tenant_id
   AND mv.branch_id = sl.branch_id
   AND mv.location_id = sl.location_id
   AND mv.ingredient_id = sl.ingredient_id
  WHERE sl.current_quantity > 0
    AND abs(mv.missing_value) >= 0.01
)
UPDATE public.stock_levels sl
SET avg_unit_cost = repair.next_wac,
    updated_at = now()
FROM repair
WHERE repair.id = sl.id
  AND repair.next_wac >= 0;
