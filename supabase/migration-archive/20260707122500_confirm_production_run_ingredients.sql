-- RPC: confirm_production_run updated to support adjusting actual ingredients
...
-- RPC: get_production_recipe_context
CREATE OR REPLACE FUNCTION public.get_production_recipe_context(p_finished_good_id bigint, p_branch_id bigint) RETURNS jsonb
    LANGUAGE plpgsql SECURITY DEFINER
    SET search_path TO 'public'
    AS $$
DECLARE
    ...
    v_res JSONB;
BEGIN
    IF NOT public.has_permission_any('inventory:production_create') AND NOT public.has_permission_any('inventory:production_confirm') THEN 
        RAISE EXCEPTION 'forbidden' USING ERRCODE = '42501'; 
    END IF;

    SELECT il.id INTO v_location_id
    FROM public.inventory_locations il
    WHERE il.branch_id = p_branch_id AND il.tenant_id = v_tenant AND il.is_default_receive = TRUE AND il.is_active = TRUE
    LIMIT 1;
    ...
    SELECT COALESCE(jsonb_agg(to_jsonb(calculated)), '[]'::jsonb) INTO v_res FROM calculated;
