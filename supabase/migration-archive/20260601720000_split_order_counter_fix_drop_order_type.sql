-- =============================================================
-- Hotfix: split_order — drop reference cột `order_type` đã không còn
-- trên `order_daily_counters` (regression từ 2026-05-14).
--
-- Lịch sử regression:
--   - 20260513000000_pos_kitchen_ticket_sequence_v2.sql DROP COLUMN
--     order_type khỏi order_daily_counters và đồng thời redefine cả
--     create_order lẫn split_order body để khớp schema mới
--     (UNIQUE (tenant_id, branch_id, counter_date), không còn order_type).
--   - 20260514000100_split_partial_skip_quota.sql add GUC bracket
--     comtammatu.skip_quota_enforcement quanh INSERT clone của partial
--     move (đúng intent), nhưng copy-paste body từ migration cũ
--     20260428021437 — phiên bản còn reference order_type. Hậu quả: từ
--     14/5 mọi gọi split_order đều RAISE 42703
--     "column order_type of relation order_daily_counters does not exist"
--     ở bước mint counter, sau khi đã pass auth/lock/validate.
--
-- Fix Option A (owner-approved 2026-05-07):
--   - Body lấy từ 20260513000000 (counter dùng 3-cột scope, prefix TC/MV
--     theo v_source.order_type) — đồng nhất với create_order hiện hành.
--   - Reapply 2 dòng PERFORM set_config('comtammatu.skip_quota_enforcement',
--     ...) quanh INSERT clone để giữ rule DAILY-LIMIT-SPLIT-NO-RECHARGE
--     (tasks/regressions.md 2026-05-01).
--
-- Không động: ACL gate, advisory_xact_lock, idempotency short-circuit,
-- FOR UPDATE source row, FOR UPDATE per item-row, full-move UPDATE
-- in-place, KDS re-point, compute_discount_amount, history audit.
-- Function signature giữ nguyên (BIGINT, JSONB, UUID) → CREATE OR REPLACE
-- đủ, không cần DROP FUNCTION.
-- =============================================================
...
BEGIN
  ...
  -- Mint new order_number from the SHARED (tenant,branch,date) counter.
  -- TC/MV is a prefix-only convention; the sequence pool is shared with
  -- create_order (see 20260513000000_pos_kitchen_ticket_sequence_v2.sql).
  ...
  LOOP
    ...
    IF v_partial_qty = v_src_row.quantity THEN
      -- Full move: in-place flip order_id. No INSERT trigger fires (UPDATE
      -- only) — quota unchanged by design.
    ...
    ELSE
      ...
      -- Net qty unchanged across {source-after, clone}, vậy daily-quota
      -- trigger MUST NOT re-charge (rule DAILY-LIMIT-SPLIT-NO-RECHARGE).
      -- Hatch: transaction-local GUC comtammatu.skip_quota_enforcement.
...
COMMENT ON FUNCTION public.split_order(BIGINT, JSONB, UUID) IS
  ...
  'p_idempotency_key. Clone INSERT bypass enforce_branch_menu_daily_limit '
  'qua GUC comtammatu.skip_quota_enforcement (net qty unchanged). Counter '
  'mint dùng schema (tenant,branch,date) — chia chung pool với create_order.';
