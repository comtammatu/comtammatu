...
SET is_delegable_to_staff = false
WHERE key = 'inventory:transfer_create';

UPDATE public.role_templates
SET permission_keys = array_remove(permission_keys, 'inventory:transfer_create'),
    updated_at = now()
WHERE position_code IS DISTINCT FROM 'owner'
  AND permission_keys @> ARRAY['inventory:transfer_create']::text[];

CREATE TEMP TABLE retired_transfer_create_grants ON COMMIT DROP AS
SELECT
  permission.id,
  ...
  owner_profile.id AS audit_actor_id
FROM public.staff_permissions permission
JOIN public.profiles target_profile
  ON target_profile.id = permission.user_id
 AND target_profile.tenant_id = permission.tenant_id
JOIN public.positions target_position
  ON target_position.id = target_profile.position_id
 AND target_position.tenant_id = target_profile.tenant_id
LEFT JOIN LATERAL (
  SELECT owner_profile.id
  FROM public.profiles owner_profile
  JOIN public.positions owner_position
    ON owner_position.id = owner_profile.position_id
   AND owner_position.tenant_id = owner_profile.tenant_id
  WHERE owner_profile.tenant_id = permission.tenant_id
    AND owner_position.code = 'owner'
    AND COALESCE(owner_profile.is_active, true) = true
  ORDER BY owner_profile.created_at, owner_profile.id
  LIMIT 1
) owner_profile ON true
WHERE permission.permission_key = 'inventory:transfer_create'
  AND target_position.code IS DISTINCT FROM 'owner';

DO $$
BEGIN
  IF EXISTS (
    SELECT 1
    FROM retired_transfer_create_grants
    WHERE audit_actor_id IS NULL
  ) THEN
    RAISE EXCEPTION 'stock_transfer_permission_revoke_requires_active_owner'
      USING ERRCODE = '23514';
  END IF;
END;
$$;
...
SELECT
  retired.tenant_id,
  retired.audit_actor_id,
  retired.user_id,
  retired.branch_id,
  'inventory:transfer_create',
  'revoke',
  retired.source_template,
  jsonb_build_object('reason', 'stock_transfer_creation_owner_only')
FROM retired_transfer_create_grants retired;

DELETE FROM public.staff_permissions permission
USING retired_transfer_create_grants retired
...
BEGIN
  ...
  IF v_role IS DISTINCT FROM 'owner' THEN
    RAISE EXCEPTION 'stock_transfer_create_owner_only' USING ERRCODE = '42501';
  END IF;
...
ALTER FUNCTION "public"."create_stock_transfer_draft"("p_from_branch_id" bigint, "p_to_branch_id" bigint, "p_transfer_number" "text", "p_notes" "text", "p_vehicle_info" "text", "p_lines" "jsonb", "p_from_location_id" bigint, "p_to_location_id" bigint) OWNER TO "postgres";

REVOKE ALL ON FUNCTION "public"."create_stock_transfer_draft"("p_from_branch_id" bigint, "p_to_branch_id" bigint, "p_transfer_number" "text", "p_notes" "text", "p_vehicle_info" "text", "p_lines" "jsonb", "p_from_location_id" bigint, "p_to_location_id" bigint) FROM PUBLIC;
GRANT ALL ON FUNCTION "public"."create_stock_transfer_draft"("p_from_branch_id" bigint, "p_to_branch_id" bigint, "p_transfer_number" "text", "p_notes" "text", "p_vehicle_info" "text", "p_lines" "jsonb", "p_from_location_id" bigint, "p_to_location_id" bigint) TO "authenticated";
GRANT ALL ON FUNCTION "public"."create_stock_transfer_draft"("p_from_branch_id" bigint, "p_to_branch_id" bigint, "p_transfer_number" "text", "p_notes" "text", "p_vehicle_info" "text", "p_lines" "jsonb", "p_from_location_id" bigint, "p_to_location_id" bigint) TO "service_role";
