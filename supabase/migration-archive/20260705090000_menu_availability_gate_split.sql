...
COMMENT ON FUNCTION public.branch_menu_limit_availability(p_tenant_id bigint, p_branch_id bigint, p_limit_date date, p_stock_gate_enabled boolean, p_exclude_hold_tokens uuid[]) IS 'stock_remaining is NULL (unlimited) both when the gate is off AND when stock_capacity_live is NULL (no recipe / broken unit config) — D064 §2 fail-open NULL doctrine, never 0. p_exclude_hold_tokens drops the caller''s own active holds from active_hold_demand so a refetch does not double-count a terminal''s own reservation.';

DROP FUNCTION public.get_branch_menu_daily_limits_for_pos(bigint);

CREATE FUNCTION public.get_branch_menu_daily_limits_for_pos(p_branch_id bigint, p_exclude_hold_tokens uuid[] DEFAULT NULL) RETURNS TABLE(menu_item_id bigint, limit_quantity integer, is_disabled boolean, sold_today integer, stock_capacity integer, stock_capacity_live integer, manual_limit_quantity integer, accepted_today integer, pending_unfinalized_demand integer, active_hold_demand integer, available_to_sell integer)
    LANGUAGE sql STABLE SECURITY DEFINER
    SET search_path TO ''
    AS $$
  WITH ctx AS (
...
COMMENT ON FUNCTION public.list_branch_menu_daily_limits(p_branch_id bigint, p_limit_date date) IS 'Manager Giới hạn bán page. gate_eff = pos_stock_availability_gate AND pos_stock_outcome_posting (D064 §1), same source as get_branch_menu_daily_limits_for_pos.';
