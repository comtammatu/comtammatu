...
-- The prior trigger rewrite compiled but failed with 42883 when GRN confirm
-- first planned the declaration expression.
...
DO $guard$
DECLARE
  v_trigger text := pg_catalog.pg_get_functiondef(
    'private.enforce_retrospective_purchase_order_line_immutability()'::pg_catalog.regprocedure
  );
BEGIN
  IF v_trigger LIKE '%pg_catalog.coalesce%' THEN
    RAISE EXCEPTION
      'enforce_retrospective_purchase_order_line_immutability still references pg_catalog.coalesce';
  END IF;
END;
$guard$;
