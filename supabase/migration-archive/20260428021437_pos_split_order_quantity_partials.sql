-- =========================================================================
-- POS tách hoá đơn — quantity-level partials (extend split_order)
--
-- Owner observation 2026-04-28: cashier không thể tách "2 Cơm sườn Cốt Lết"
-- (1 row qty=2) thành 2 bill mỗi bill 1 phần. Existing split_order chỉ
-- move WHOLE rows; gate `activeItemCount >= 2` đếm rows nên 1 row qty=2
-- không mở được sheet. Khách trả riêng theo phần là use-case phổ biến
-- nhất của Tách → bug nghiêm trọng cho UX.
--
-- Fix:
--   - Signature: split_order(BIGINT, BIGINT[], UUID)
--                → split_order(BIGINT, JSONB, UUID)  -- [{item_id, quantity}]
--   - Per partial:
--     - quantity == row.quantity → in-place UPDATE order_id (như cũ:
--       row + kds_tickets giữ id, theo dõi đơn mới qua order_id flip).
--     - quantity <  row.quantity → INSERT order_items mới trên đơn mới
--       (clone menu_item_id/variant/modifiers/sides/note/status/
--       sent_to_kitchen_at) + UPDATE source row giảm quantity, recompute
--       subtotal. Mirror kds_tickets: 1 ticket per (order_item, station),
--       nên cần INSERT ticket mới cho row mới — copy status/bumped_*/
--       created_at để chef không bị reset cooking timer.
--   - "Would empty source" check chuyển sang đếm rows-after-move (chỉ
--     full-move xoá row khỏi source; partial-move giữ row với qty giảm).
-- =========================================================================

-- ─── Drop cũ + create mới ──────────────────────────────────────────────

DROP FUNCTION IF EXISTS public.split_order(BIGINT, BIGINT[], UUID);
...
BEGIN
  ...
  -- ─── Validate partials + lock source rows + count full-moves ─────────
  -- Pre-pass: enforce shape, ownership (item belongs to source + not
  -- cancelled), qty bounds (1..row.quantity). Track full-move count for
  -- the "would empty source" check before committing any change.
  ...
  -- Source must keep >=1 active ROW after move. Partial-moves leave the
  -- source row in place (just qty giảm); full-moves remove it from source.
  ...
  -- ─── Mint new order_number (same TC/MV counter as create_order) ─────
  ...
  LOOP
    ...
    IF v_partial_qty = v_src_row.quantity THEN
      -- Full move: in-place flip order_id. Item id giữ nguyên, kds_tickets
      -- theo qua order_id update — chef thấy ticket xuất hiện ở đơn mới
      -- mà không reset thời gian chờ/cooking.
      ...
      UPDATE public.kds_tickets
       ...
       WHERE order_item_id = v_partial_item_id
         AND order_id = p_source_order_id;
    ELSE
      -- Partial move: clone row sang đơn mới với phần qty, giảm source row.
      -- Modifiers/sides/note/status/sent_to_kitchen_at copy as-is — món
      -- vật lý vẫn cùng cấu hình. request_key=NULL (không phải append).
      ...
      -- Mirror kds_tickets: UNIQUE(order_item_id, station_id, tenant_id),
      -- new order_item cần ticket mới ở mỗi station mà source row đã có.
      -- Copy status + bumped_*+ created_at để chef không thấy card
      -- "fresh" cho món đã đang cooked. updated_at default now() để
      -- realtime publication (REPLICA IDENTITY FULL) bắn UPDATE event.
  ...
  -- ─── Recompute SOURCE totals ────────────────────────────────────────
  -- POS-DISCOUNT-RECOMPUTE-VIA-HELPER: pct re-applies trên subtotal mới,
  -- vnd auto-clamp về subtotal nên total_amount không âm.
  ...
  -- ─── Recompute NEW totals (no discount, no service charge) ──────────
...
REVOKE ALL ON FUNCTION public.split_order(BIGINT, JSONB, UUID) FROM PUBLIC;
GRANT EXECUTE ON FUNCTION public.split_order(BIGINT, JSONB, UUID) TO authenticated;

COMMENT ON FUNCTION public.split_order(BIGINT, JSONB, UUID) IS
  'Tách hoá đơn — partials [{item_id, quantity}]. quantity == row.quantity '
  '→ in-place UPDATE order_id; quantity < row.quantity → INSERT clone trên '
  'đơn mới + giảm qty source + mirror kds_tickets per station. Source phải '
  'giữ ≥1 active ROW sau move (block split_would_empty_source). Đơn mới: '
  'discount=0, service_charge=0, status inherit từ source. Idempotent qua '
  'p_idempotency_key.';
