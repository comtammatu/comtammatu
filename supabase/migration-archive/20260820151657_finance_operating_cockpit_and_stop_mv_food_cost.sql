-- Period operating cockpit: one round-trip for /finance landing KPIs.
-- Funds stay on get_finance_current_funds. Theoretical food cost stays off-hub.
-- Also stop refreshing the unused mv_food_cost view.
...
BEGIN
  IF auth.uid() IS NOT NULL
    AND NOT public.has_permission_any('settings:tenant')
  THEN
    RAISE EXCEPTION 'permission denied: settings:tenant required'
      USING ERRCODE = '42501';
  END IF;
  -- mv_food_cost is no longer an app read path. Keep the RPC so the
  -- owner refresh button and any leftover cron do not fail.
END;
$$;
