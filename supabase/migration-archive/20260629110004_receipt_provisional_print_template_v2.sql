...
WITH headers AS (
  SELECT
    pt.id,
    CASE
      WHEN pt.kind = 'receipt'
       AND pt.content#>>'{blocks,0,type}' = 'row'
       AND pt.content#>>'{blocks,1,type}' = 'row'
      THEN jsonb_build_array(
        jsonb_build_object('type', 'brandHeader', 'eyebrow', 'TIỆM CƠM TẤM', 'name', 'MÁ TƯ', 'tagline', 'Thịt tươi 100%'),
        jsonb_build_object('type', 'branchInfo')
      ) || (((pt.content->'blocks') - 0) - 0)
      ELSE pt.content->'blocks'
    END AS blocks
  FROM public.print_template_versions pt
  WHERE pt.kind IN ('receipt', 'provisional_bill')
...
flagged AS (
  ...
  FROM headers h
  CROSS JOIN LATERAL jsonb_array_elements(h.blocks) WITH ORDINALITY AS b(block, ord)
  GROUP BY h.id
)
UPDATE public.print_template_versions pt
SET content = jsonb_set(pt.content, '{blocks}', flagged.blocks),
    updated_at = now()
FROM flagged
WHERE pt.id = flagged.id
  AND pt.content->'blocks' IS DISTINCT FROM flagged.blocks;
