-- Follow-up to 20260628140000: once the webhook (service_role) can enqueue a
-- receipt, the existing ON CONFLICT clause flipped an already-'printed' job back
-- to 'pending', so a cash->vietqr correction (confirm_sepay_payment Path A, where
-- the cashier already printed the cash receipt) would reprint. Gate the
-- printed->pending revival behind NOT service_role: an explicit cashier reprint
-- (pos:reprint_receipt) still revives a printed job, but the webhook never
-- reprints one. service_role still retries 'failed'/'expired' jobs, and a not-yet
-- printed ('pending') job still picks up the corrected payload and prints once.
