...
WITH CHECK (
  ...
  AND (
    (
      (storage.foldername(name))[2] = 'grn'
      AND EXISTS (
        SELECT 1
        FROM public.goods_received_notes AS grn
        WHERE grn.id = CASE
            WHEN COALESCE((storage.foldername(name))[3], '') ~ '^[1-9][0-9]*$'
              THEN (storage.foldername(name))[3]::bigint
          END
    ...
    OR (
      (storage.foldername(name))[2] = 'stock-issues'
      AND EXISTS (
        SELECT 1
    ...
    OR (
      (storage.foldername(name))[2] = 'branches'
      AND (storage.foldername(name))[4] = 'waste'
      AND EXISTS (
        SELECT 1
    ...
    OR (
      (storage.foldername(name))[2] = 'waste'
