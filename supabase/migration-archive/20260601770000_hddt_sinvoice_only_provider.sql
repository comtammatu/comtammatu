-- =============================================================
-- HĐĐT provider cleanup: Viettel S-invoice only.
--
-- MISA meInvoice is no longer a runtime provider for this project. Keep
-- historical rows untouched, but new DB defaults and daily-summary drafts
-- must use provider='viettel'.
-- =============================================================

ALTER TABLE public.tax_invoices
  ALTER COLUMN provider SET DEFAULT 'viettel';

COMMENT ON COLUMN public.tax_invoices.provider IS
  'HĐĐT provider. Runtime supports Viettel S-invoice only; historical rows may contain misa/mock/skipped.';

UPDATE public.tax_invoices
SET provider = 'viettel'
WHERE provider = 'misa'
...
REVOKE ALL ON FUNCTION public.aggregate_daily_b2c_invoice(BIGINT, DATE, UUID)
  FROM PUBLIC, anon;
...
COMMENT ON FUNCTION public.aggregate_daily_b2c_invoice(BIGINT, DATE, UUID) IS
  'Daily B2C summary HĐ aggregation. Creates draft tax_invoices row with provider=viettel and returns line_items_for_provider for the Node Sinvoice caller.';
