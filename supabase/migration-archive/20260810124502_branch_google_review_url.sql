-- Branch Google Review URL for post-feedback routing (Wave 1.2).
-- Self-Order snapshot exposes phone + googleReviewUrl for guest CTAs.
...
BEGIN
  IF NOT EXISTS (
    SELECT 1
    FROM pg_constraint
    WHERE conname = 'branches_google_review_url_chk'
  ) THEN
    ALTER TABLE public.branches
      ADD CONSTRAINT branches_google_review_url_chk
      CHECK (
        google_review_url IS NULL
        OR (
          char_length(google_review_url) BETWEEN 1 AND 500
...
COMMENT ON COLUMN public.branches.google_review_url IS
  'Optional Google Maps Place/Review URL for the branch. Used after guest rating >= 4.';
