-- =============================================================
-- Fix: transfer_order_table — 'served' is NOT a terminal status
-- =============================================================
-- The original 20260429100000_transfer_order_table_allow_occupied
-- excluded 'served' from the active-on-old count when releasing the
-- source table. That conflicts with regression rule
-- POS-SERVED-NOT-TABLE-TERMINAL: `served` only means fulfillment is
-- done, the bill may still be unpaid. The table-release trigger
-- (`trg_release_table_on_order_status`) treats only `completed` and
-- `cancelled` as terminal — this RPC must agree.
--
-- Failure mode without fix: bàn A has order O1 (served, unpaid) and
-- order O2 (preparing). Transferring O2 to bàn B sees v_active_on_old = 0
-- (because O1 is 'served' and excluded), then frees bàn A while O1 is
-- still attached → bàn A reused while an active order exists.
...
BEGIN
  ...
  -- Reserved / maintenance still block.
  ...
  IF v_old_table_id IS NOT NULL THEN
    -- Terminal set MUST match `trg_release_table_on_order_status` =
    -- (completed, cancelled). 'served' stays active here so a transfer
    -- can't free a bàn that still has another served-but-unpaid order.
