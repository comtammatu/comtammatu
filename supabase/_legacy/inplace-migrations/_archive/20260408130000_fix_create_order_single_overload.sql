-- =============================================================
-- Fix: PostgREST 403 / Postgres 42501 on create_order
--
-- PostgreSQL treats CREATE OR REPLACE with a *new* parameter list as
-- a *new overload*, not a replacement. If DROP FUNCTION did not match
-- the exact signature (e.g. INT vs integer), a stale 9-arg create_order
-- could remain without EXECUTE for authenticated → 42501 on RPC.
-- This migration drops ALL public.create_order overloads and recreates
-- exactly one 10-arg function + GRANT.
-- =============================================================
