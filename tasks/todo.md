...
- [ ] **Separate DB follow-up — Owner predicate execution.** A fresh local
      migration chain required a local-only authenticated `EXECUTE` grant on
      `public.auth_is_owner(uuid)` before the existing `refunds_select` policy
      could load matched refunds. Resolve the policy/function boundary in a
      dedicated DB lane, prove fresh-chain and target-ledger behavior, and seek
      explicit owner delegation before any production apply. This UI lane adds
      no migration, grant, RPC, or production write.
