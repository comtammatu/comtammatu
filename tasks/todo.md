...
Evidence: `finance-expenses.test.ts`.
- [ ] Owner reviews purpose-first rows and startup kind filter on `/finance/expenses`
## Split construction from capital assets
State: verify
Exit: Separate `capital`/`construction`; construction outside total operational value.
...
Evidence: `ShopeeReceiptPipelineTest`; `DeliveryPlatformDetectorTest`; `check-operational-tools` Shopee-only guard.
- [ ] Install Agent 1.7.2 on Redmi and confirm one ShopeeFood job OCR-classifies, posts to POS, and keeps bitmap/OCR until cleanup
- [ ] Confirm Device shows only ShopeeFood intake and Agent logs have no other-platform transport checks
UI Advisor Gate
- Surface: native Agent BOARD/LIST; overview → OCR → POS; Redmi intake + `corepack pnpm verify`
...
Evidence: `pwa-device-compatibility.test.ts` RED → GREEN; compiled CSS viewport model passes at 360/390/820/1024px; 37 PWA tests and full `pnpm test` pass; `corepack pnpm verify` passes (build replayed from Turbo cache). Device proof remains pending; local authenticated runtime lacks `NEXT_PUBLIC_SUPABASE_URL`.
...
- [ ] Run the runbook matrix on Android/HyperOS and iOS: capture browser/version, keyboard/viewport, overlay, asset errors, recovery time, stale data, refetch count, and duplicate sockets
...
Evidence: static tests `vietqr-auto-refresh-flow`, `pos-table-order-visual-state` green. Production `enloyfnuerqgaqderbwb` ledger has `20260903120620`; refresh function and `orders` trigger are live; `db:types` regenerated.
