...
- [~] **Ops reconciliation Momo desync** — ✅ logic ĐÃ CÓ: RPC `find_payment_order_desync(p_since)` (baseline:9008, gate `finance:view`, detect completed-payment + order≠paid); verify prod = **0 desync hiện tại**. Còn: surface lên cockpit `/finance` (thêm 1 `FinanceException`). **Kẹt:** copy phải vào `lib/messages/finance.ts` đang bị phiên khác sửa dở → chờ tree sạch rồi thêm (≈10 dòng ở `finance-cockpit.ts` + copy).
