...
- [ ] **WS-3 — decomposition (NOT started).** Goal: each file owns ONE clear concern, **measured by cohesion, not a line count** (see memory `feedback-refactor-goal-separation-not-loc`). Split monolithic files along responsibility lines + a `"use server"` barrel re-export so caller import paths don't change:
  - **Client shells** → `_hooks/` (state / realtime / actions) + `views/`: `pos-desktop-shell` (~1.7k), `grn-detail-client` (~1.5k), `order-detail-sheet` (~1.5k).
  - **Action files** → split by feature group: `order-actions.ts` → reads / lifecycle / void-cancel / priority / transfer; inventory `grn-actions` / `production-actions` / `actions.ts`.
  - Extract remaining inline schemas / messages / formatters into `_lib/`.
