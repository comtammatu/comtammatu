...
- [ ] **`cancel_pending_payment` in-function authz** — cùng class `create_payment` (D043): definer, GRANT authenticated, nhận `p_tenant_id`/`p_branch_id` không verify, không permission check → user cancel pending payment chi nhánh bất kỳ. Fix: thêm `p_tenant_id = auth_tenant_id()` + `has_permission(branch, 'pos:use')`. CI scan DEFINER-authz nên bắt nó; hand-fix nếu làm trước. T3 (money).
