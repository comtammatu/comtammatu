...
- [ ] **Reconcile remaining e2e specs (now that the harness exists, #110)** — these run under `scripts/supabase-e2e-bringup.mjs` but each drifted (never run before). Verify each against current code/behavior, fix, then add to the gate (`apps/web/package.json` `test:e2e:smoke`). One spec per PR.
  ...
  - `daily-limit-realtime.spec.ts` — tz handling **ALREADY FIXED** (the enforcement test now derives `limit_date` via `Intl.DateTimeFormat("en-CA", { timeZone: "Asia/Ho_Chi_Minh" })`, `daily-limit-realtime.spec.ts:89-91`); the old "UTC `limit_date`" note above is stale. Remaining: verify by actually running the spec under the bring-up harness (currently not in the gate), and the broadcast test remains flaky.
  ...
  - `inventory/*.spec.ts` — DB-contract scenarios pass; UI specs (`grn-procurement`, `transfer-direction`) **ALREADY use `storageState: E2E_AUTH_STORAGE_MANAGER`** and `test.skip(true, …)` defensively when the `warehouse_manager` fixture is missing — the old "UI scenarios `test.skip` under the cashier storageState" note is stale. The `E2E_INVENTORY_MANAGER_*` env is wired. Remaining real work: seed a `warehouse_manager` profile + `ingredients` row (tenant 1) for `issue-label-by-branch-kind` test 3, then add to the gate once the CI multi-spec blocker below is resolved.
