...
- [ ] **Residual broad grants** — 1 migration `REVOKE TRUNCATE/REFERENCES/TRIGGER FROM anon, authenticated ON ALL TABLES` (prod: anon giữ trên 101 bảng, authenticated 102; cosmetic, KHÔNG exploit qua PostgREST). `bmidl_write` legacy `auth_role()` gộp vào α4c.
- [ ] **HRM Đợt 2** (D026, không cần owner) — (a) tạo NV 1 bước (RPC atomic `createUser` + insert `employees`, bỏ dán UUID ở `employee-form-dialog.tsx`) + `updateEmployee` + ngưng việc (`is_active`+`end_date`) + SĐT; (c) nghỉ phép notify 2 chiều + quick-action "Xin nghỉ" + gộp pending toàn-CN; (d) đổi nhãn `/admin/staff` → "Tài khoản & phân quyền".
