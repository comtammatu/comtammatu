...
14. **Learn from the Matu Superapp design-system precedent before POS/KDS UI work**
    - Pattern: POS/KDS visual fixes were made from local symptoms and project rules, but the stronger product precedent in sibling repo `~/matu-superapp` was not consulted early enough.
    - Rule: Before UI/UX rebuild, design polish, or frontline layout changes, read `~/matu-superapp/DESIGN.md`, `~/matu-superapp/docs/spec/design-system.md`, `~/matu-superapp/docs/agent/rules/ui.md`, and `~/matu-superapp/docs/architecture/design-tokens-bridge.md` when available. Use it as the reference for Má Tư minimalist warm operational UI: semantic tokens, border-first surfaces, 8px default radius, restrained shadows, 44px frontline touch targets, Vietnamese operational copy, and first viewport focused on next action/live queue.
    - Prevention: Add a UI-plan checklist item: "Matu Superapp reference checked?" If current runtime contract differs, update or reconcile `docs/spec/design-system.md` first instead of copying styles ad hoc.
