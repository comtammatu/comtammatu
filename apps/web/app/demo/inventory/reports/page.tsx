...
import { reportData, formatVND } from "../_components/mock-data";

export default function ReportsPage() {
  const barChartData = reportData.movementSummary.map((m) => ({
    label: m.month,
    values: [
  ...
  const maxAP = Math.max(...reportData.apAging.map((a) => a.amount));
      ...
      <div className="grid grid-cols-12 gap-6">
        ...
        >
          ...
          <div className="space-y-4">
            {reportData.apAging.map((item, idx) => {
              const isOverdue = idx === 3;
              const barColor =
                idx === 0
                  ? "var(--md-secondary)"
                  : idx === 1
        ...
        >
          ...
          <div className="space-y-4">
            {reportData.consumptionVariance.map((item) => {
        ...
        >
          ...
          <TrendSparkline
            data={reportData.foodCostTrend}
            width={400}
            height={120}
            color="var(--md-primary)"
            target={reportData.foodCostTarget}
          />
          <p
