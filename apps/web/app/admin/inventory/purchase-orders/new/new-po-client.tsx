...
export function NewPoClient({
  ...
  // Price deviation state: map ingredientId → deviation info
  const [lineDeviations, setLineDeviations] = useState<
    Map<number, SinglePriceDeviation>
  >(new Map());
  // Inline deviation for add-row form (the row being composed)
  const [addRowDeviation, setAddRowDeviation] =
    useState<SinglePriceDeviation | null>(null);

  // Check deviation for a single ingredient when price is entered
  function checkPriceDeviation(
    ...
    if (!supplierId || price <= 0) {
      if (target === "addRow") setAddRowDeviation(null);
      return;
    }
    fetchSinglePriceDeviation({
      ingredientId: ingId,
      ...
      if (target === "addRow") {
        setAddRowDeviation(dev);
      } else {
        setLineDeviations((prev) => {
          const next = new Map(prev);
          if (dev) next.set(ingId, dev);
  ...
  // Set of ingredient IDs already in PO lines
  ...
  function addLine(e: React.FormEvent<HTMLFormElement>) {
    ...
    // Check deviation for added line if it has a price
    if (unitPriceEst != null && unitPriceEst > 0) {
      checkPriceDeviation(iid, unitPriceEst, "line");
    }
    setIngredientId("");
    setUnit("");
    setAddRowDeviation(null);
  ...
  const addableCount = suggestions.filter(
    (s) => !lineIngredientIds.has(s.ingredient_id) && s.suggested_qty > 0,
  ).length;
                      ...
                      >
                        ...
                        <div className="col-span-2 flex justify-end">
                          <Button
                            ...
                            onClick={() => addSuggestionToLines(s)}
                          >
                            {alreadyAdded ? (
                              "Đã thêm"
                            ) : (
          ...
          <div className="pl-2">
            <Input
              ...
              onBlur={(e) => {
                ...
                if (ingId && price > 0) {
                  checkPriceDeviation(ingId, price, "addRow");
                } else {
                  setAddRowDeviation(null);
                }
              }}
...
// ---------------------------------------------------------------------------
// Inline deviation hint — shows below price field
// ---------------------------------------------------------------------------
