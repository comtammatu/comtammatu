...
export const SHORTAGE_REASONS = [
  {
    value: "discrepancy",
    label: INVENTORY_VI.shortageReasonDiscrepancy,
    needsPhoto: false,
  },
  { value: "loss", label: INVENTORY_VI.shortageReasonLoss, needsPhoto: false },
  {
    value: "spoiled",
    label: INVENTORY_VI.shortageReasonSpoiled,
    needsPhoto: true,
  },
] as const;
...
export function CountSlipWasteEvidence({
  ...
  return (
    <NoteCallout
      tone="warning"
      label={compact ? undefined : INVENTORY_VI.countSlipWasteEvidenceTitle}
        ...
        <div className="min-w-0">
          {compact ? (
            <div className="flex min-w-0 items-center gap-2">
              <span className="truncate text-xs font-semibold uppercase tracking-wide text-warning">
                {INVENTORY_VI.countSlipWasteEvidenceTitle}
              </span>
              <span
                className="shrink-0 rounded-full bg-warning/15 px-2 py-0.5 text-xs font-semibold text-warning"
                aria-label={INVENTORY_VI.countSlipWasteEvidenceShortageCount(
                  lines.length,
                )}
              >
                {lines.length}
                  ...
                  {onReasonChange ? (
                    <div className="flex flex-col gap-1">
                      ...
                      >
                        ...
                        >
                          <SelectValue
                            placeholder={INVENTORY_VI.shortageReasonLabel}
                          />
                        </SelectTrigger>
                        <SelectContent>
                          {SHORTAGE_REASONS.map((r) => (
