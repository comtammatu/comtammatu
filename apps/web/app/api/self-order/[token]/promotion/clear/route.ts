...
import { selfOrderPromotionClearRequestSchema } from "@lib/self-order/contracts";
import {
  clearSelfOrderPromotion,
...
export async function POST(
  ...
  if (!validateSelfOrderMutationRequest(request)) {
    return jsonError(403, "forbidden", SELF_ORDER_VI.promoClearFailed);
  }

  const body = await parseJsonBody(request);
  const parsed = selfOrderPromotionClearRequestSchema.safeParse(body);
  if (!parsed.success) {
    return jsonError(422, "invalid_body", SELF_ORDER_VI.promoClearFailed);
  }

  const result = await clearSelfOrderPromotion({
    token,
