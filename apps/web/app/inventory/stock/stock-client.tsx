...
export function StockClient({
  ...
  return (
    <InventoryPageContent
      ...
      density="compact"
      mobile={isMobile}
      contentClassName={isMobile ? undefined : "max-w-none gap-3"}
      scroll
    >
      <AppPageHeader density="compact" title={stockCopy.title} />
      <div className="flex flex-wrap items-center gap-2 border bg-card p-2">
        ...
        {permissions.canStartProduction ? (
          <QuickActionButton
            href={branchHref(branchId, "/inventory/production")}
            icon={IconFactory}
            label={stockCopy.actions.startProduction}
          />
        ) : null}
