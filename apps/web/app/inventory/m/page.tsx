...
async function fetchHubCounts(): Promise<{
  ...
  if (!session) return { openPoCount: 0, pendingTransferCount: 0 };
  const claims = extractClaims(session.user.app_metadata);
  if (!claims) return { openPoCount: 0, pendingTransferCount: 0 };

  const [poRes, tfRes] = await Promise.all([
    supabase
...
export default async function InventoryMobileHub() {
  ...
  const role = session
    ...
    : undefined;
  const showProcurement = role ? canAccess(role, "inventory_procurement") : false;

  const { openPoCount, pendingTransferCount } = await fetchHubCounts();
