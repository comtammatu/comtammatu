...
export function IssuesClient({ issues }: { issues: IssueRow[] }) {
  ...
  const kitchenUseCount = issues.filter((i) => i.type === "kitchen_use").length;
  const writeOffCount = issues.filter((i) => i.type === "write_off").length;
            ...
            {filtered.map((item) => (
              ...
              >
                <TableCell className="px-6 py-5">
                  <Link
                    href={`/inventory/issues/${item.code}`}
