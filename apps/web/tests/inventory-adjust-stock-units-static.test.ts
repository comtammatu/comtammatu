...
import { readActiveMigrationSql, assertSqlMatch } from "./_lib/active-sql.ts";
...
test("stock adjustments preserve the selected issue or receipt unit through the RPC", () => {
  ...
  assertSqlMatch(migration, /ingredient\.issue_unit_id/);
  assertSqlMatch(migration, /ingredient\.receipt_unit_id/);
  assertSqlMatch(migration, /entry_to_base_factor/);
  assertSqlMatch(migration, /entry_unit_code/);
  assertSqlMatch(migration, /IF v_need_qty = 0 THEN\s+CONTINUE;/);
  assertSqlMatch(migration, /ingredient\.production_unit_id/);
  assertSqlMatch(migration, /v_need_qty \/ v_raw_entry_to_base_factor/);
  assert.match(
    stockClient,
