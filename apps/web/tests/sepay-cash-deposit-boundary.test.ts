...
test("cash deposit is a matched internal transfer, not customer money-in", () => {
  assert.equal(classifySepayReconciliationState(matchedCashDeposit()), "matched");
...
test("cash deposit has one verified atomic write boundary", () => {
  const migration = read(
    "supabase/migrations/20260713150807_harden_sepay_cash_deposit_boundary.sql",
  );
  ...
  assert.match(route, /status: z\.enum\(\["recorded", "already_recorded"\]\)/);
  assert.match(route, /expense_id: z\.number\(\)\.int\(\)\.positive\(\)/);
  assert.match(route, /error_code: "bank_deposit_result_invalid"/);
  assert.match(route, /return NextResponse\.json\(\{ success: false \}, \{ status: 500 \}\)/);
});
