...
test("owner inventory report starts its independent report loaders together", () => {
  ...
  assert.match(
    page,
    /const \[apRes, varRes, movementRes, foodCostRes, wasteRes[\s\S]*?\] = await Promise\.all\([\s\S]*fetchApAging\(\)[\s\S]*fetchConsumptionVariance\([\s\S]*fetchStockMovementReport\([\s\S]*fetchFoodCost\([\s\S]*loadWasteAnalyticsData\(/,
  );
});
