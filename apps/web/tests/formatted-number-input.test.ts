...
test("decimal input keeps a Vietnamese trailing separator while typing", () => {
  ...
  assert.deepEqual(
    parseVietnameseNumericInput(",", { maxFractionDigits: 3 }),
    {
      state: "incomplete",
      display: "0,",
    },
...
test("decimal input accepts only the Vietnamese comma separator", () => {
  assert.deepEqual(
  ...
  assert.equal(
    parseVietnameseNumericInput("8.8", { maxFractionDigits: 3 }).state,
    "invalid",
  );
});

test("invalid decimal drafts use native validation and never emit blur values", () => {
  ...
  assert.match(source, /draft\?\.state === "invalid"[\s\S]*else \{[\s\S]*onValueBlur/);
  assert.doesNotMatch(source, /normalizeTypedDecimalPointAlias/);
});

test("decimal fields distinguish Vietnamese grouping from decimal comma", () => {
  assert.deepEqual(
...
test("mixed locale paste accepts vi-VN and rejects dot decimals", () => {
  assert.deepEqual(
  ...
  assert.equal(
    parseVietnameseNumericInput("1,234.56", { maxFractionDigits: 3 }).state,
    "invalid",
  );
  assert.equal(
