...
test("Wave 1 orders tab can Tạo đơn; YCM tab stays readable without create", () => {
  ...
  assert.match(client, /changeSupplier|setDraftLines\(\[blankRequestLine\(\)\]\)/);
  assert.match(client, /unmappedSendBlocked/);
  assert.match(form, /variant="document"/);
  assert.match(form, /selectSupplierFirst/);
  assert.match(form, /mappedIngredientIds|supplierIngredients/);
  assert.match(form, /disabled=\{!hasSupplier\}/);
  assert.match(form, /disabled=\{!canEditLines\}/);
  assert.match(form, /data-locked/);
  assert.match(form, /pointer-events-none opacity-50/);
  ...
  assert.match(copy, /selectSupplierFirst:/);
