...
test("list scope -> embedded routeBranchId and Admin Dashboard ?branchId= resolve through the identical selectBranchScope call", async () => {
...
test("list scope -> Admin Dashboard ?branchId= never triggers outOfScope, only clamps the selection", async () => {
  const scope = await resolveInventoryListScope(
    fakeSupabase(BRANCHES, null).supabase,
    claims("cashier", 2),
    { queryBranchId: "1" },
