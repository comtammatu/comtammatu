...
test("direct GRN creation routes operators back to purchase orders", () => {
  ...
  assert.match(page, /redirect\("\/inventory\/purchase-orders"\)/);
  assert.doesNotMatch(page, /GrnCreateClient|loadGrnCreatePageData/);
  assert.match(supplierPage, /redirect\("\/inventory\/grn\/new"\)/);
});
