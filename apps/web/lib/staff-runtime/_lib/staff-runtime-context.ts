 ...
 *
 * Wrapped in React cache() so multiple components or Server Actions within
 * the same RSC render share one lookup instead of repeating DB queries.
 */
export const getEmployeeContext = cache(
  async function getEmployeeContext(): Promise<EmployeeContext | null> {
    const state = await loadAuthState();
    const { supabase, claims, userId } = state;
