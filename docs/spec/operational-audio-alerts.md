...
resolved via `branch_settings` -> `system_settings` -> `openai/tts-1 onyx` fallback)
at recorded speed, peak-normalized, presence-EQ'd, then peak-limited (no OS TTS). Fetch with the beep
