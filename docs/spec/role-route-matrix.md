...
| Inventory Oversight | `/inventory/*`                                                                                                                                     | L0      | `owner`; `accountant` only for YCM/PO/GRN; central roles by explicit grant | Tenant inventory, central YCM→PO→GRN (D096), stock requests inbox, stocktake, production (central kitchen), consumption, waste, and reports. Branch daily work: `/br/[branchId]/stock/*` (yêu cầu hàng, không GRN/SX). |
