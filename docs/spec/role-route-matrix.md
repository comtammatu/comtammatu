...
| Inventory Oversight | `/inventory/*`                                                                                                                                     | L0      | `owner`; `accountant` only for GRN/PO; central roles by explicit grant | Tenant inventory, central GRN→PO (D093), stock requests inbox, stocktake, production (central kitchen), consumption, waste, and reports. Branch daily work: `/br/[branchId]/stock/*` (yêu cầu hàng, không GRN/SX). |
