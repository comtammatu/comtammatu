...
| Inventory Oversight      | `/inventory/*`                                                                                                                                     | L0      | `owner`; `accountant` only for PO/GRN; central roles by explicit grant | Tenant inventory, purchase demand allocation → PO → GRN, stock requests inbox, stocktake, production (central kitchen), consumption, waste, and reports. Branch daily work: `/br/[branchId]/stock/*` (warehouse-to-kitchen transfers, stocktake/count, requests; no GRN/production). |
