...
- **Form Layout:** Responsive multi-column forms inside dialogs and sheets use canonical `FormGrid` and `FormRow` (`packages/ui/src/components/form-grid.tsx`, re-exported as `AppFormGrid` and `AppFormRow` via `apps/web/app/components/form/app-form-grid.tsx`), automatically adjusting from 1 column on mobile to 2 columns on desktop (`density="compact"` for dialogs, `density="default"` for pages).
- **Navigation Link Cards:** Hub and dashboard navigation cards use canonical `AppLinkCard` and `LinkCardGrid` (`packages/ui/src/surface/link-card.tsx`, re-exported via `apps/web/app/components/surface/app-link-card.tsx`), providing structured semantic icon badge, metric/status display, and accessible keyboard focus with decoupled route linking.
