...
> Reconciled-through main @ c83875ac (2026-07-04). Owner-driven reset of Phước Hải inventory.
...
6. ✅ Units registry canonicalized (owner: "trùng lặp đơn vị, quy tắc chung, lowercase, bỏ song song code/name, editable không hardcode", 2026-07-04). PROD data cleaned (guard-toggle): 24→20 units, 4 accent-orphans deleted, stripped codes re-accented (goi→gói/thung→thùng/trai→trái/vi→vỉ), `name=code` for all, `recipes.unit` piece→cái, `supplier_price_list.uom` vi→vỉ. Hardening migration `20260703170000_units_canonical_hardening` merged (#229) + **APPLIED to PROD** (self-heal no-ops + `units_code_lowercase_chk CHECK (code=lower(code))`); normalize-on-write in units-actions (lowercase+trim, name=code); units CRUD confirmed DB-driven (not hardcoded). Root cause of the accent-dups: the Phase A seed's ON CONFLICT was on exact (code,tenant_id) so accented seed codes did not collide with the pre-existing stripped codes.
7. ⛔ CANCELLED — Re-enter opening balance + turn deduction back ON. Owner decision 2026-07-04: **"tắt trừ kho bán nên không cần tồn đầu kỳ GRN"** — deduction stays OFF (D064: DED off ⇒ capacity NULL ⇒ sell freely), so there is no opening-balance re-entry now. Inventory stays empty by choice. Consequently the "nhập đầu kỳ" trigger (D060 §5) is NOT firing → ledger-fix, Phase B (11-RPC anchor rewrite + freeze-base), and `verify_inventory_ledger` RPC all remain PARKED until the owner opts to re-enable deduction (D064 runbook: GRN đầu kỳ → DED shadow → GATE per branch).

## Conclusion (2026-07-04)

Reset achieved its purpose: the known-wrong Phước Hải inventory is wiped clean, deduction is off (POS sells freely per D064), and the units registry is canonical + constraint-locked. No opening-balance rebuild happens now — that is the owner's chosen steady state. The unit-system anchor model (Phase A) is live-but-dormant; Phase B stays deferred with no live ledger to serve.
