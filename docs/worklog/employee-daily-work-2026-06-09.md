# Employee Daily Work - T3 Contract
...
- Motion/effect role: none beyond native camera capture and lightweight pending state.
...
- Acceptance is one primary CTA per state, photo-only clock-in, binary checklist, manager-approval checkout, and no GPS in Employee.
- Admin config can be minimal: checklist items per branch.
...
- Flow tests must cover no photo, wrong type/too-large photo, duplicate clock-in, checklist persistence, checkout guard, manager approval request, no GPS, and fallback checklist.
...
Implement Employee Daily Work with these non-negotiables:
...
5. Checkout sends a pending request for manager approval; staff do not scan QR or enter a code.
...
8. Camera permission is allowed for Employee photo capture while geolocation remains disabled.
