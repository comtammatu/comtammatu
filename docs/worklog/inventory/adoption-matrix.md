...
| Production / central kitchen                                                    | ✅  | ✅   | ⚠️     | `Stable`   | Production operator là `super_manager` / `production_manager`; `owner` có access kiểm tra/khẩn cấp; `area_manager`/`branch_manager` bị chặn ở Server Action + DB/RPC/RLS dù có manual grant. Modal create order còn warning a11y và chưa verify blocked path thiếu BOM/nguyên liệu |
