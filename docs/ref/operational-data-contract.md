...
| `finance.revenue.monthly_target_reward`     | Mốc thưởng KPI          | Khi chi nhánh đạt mốc KPI thì mức thưởng nào được áp dụng?                 | `branch_revenue_targets.reward_tiers`; mỗi mốc có `% KPI`, `fixed_amount` hoặc `revenue_percent`, và giá trị thưởng. Owner ghi cùng chỉ tiêu. BM đọc mốc của branch gán qua `get_branch_revenue_target_progress.reward_tiers` (không qua list RPC Owner). | Mốc cao nhất đã đạt được áp dụng, không cộng dồn; `% doanh thu` tính trên Doanh thu thuần tháng. Đây là cấu hình quỹ thưởng chi nhánh, chưa tự phân bổ vào payroll. |
| `finance.revenue.monthly_target_progress`   | Tiến độ chỉ tiêu         | Chi nhánh đã đạt bao nhiêu % chỉ tiêu Doanh thu thuần tháng này? Doanh thu ngày bao nhiêu? | `finance.revenue.before_vat_after_discount` trong tháng / ngày lịch VN / `finance.revenue.monthly_target`; MTD hoặc đúng một tháng lịch. | `trusted` khi có target; thiếu target không hiện 0%; BM chỉ xem đúng branch được gán trên Branch home (Tháng \| Ngày + Progress + mốc). |
