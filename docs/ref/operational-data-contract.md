...
| `finance.expense.operating`                 | Chi phí vận hành        | Kỳ này đã ghi nhận bao nhiêu chi phí vận hành?                             | `expenses.amount` thuộc `rent`, `utilities`, `gas_fuel`, `salary`, `repair`, `supplies`, `marketing`, `fees_tax`, `other`; `supplies` chỉ chứa vật tư tiêu hao/công cụ nhỏ hoặc phần phân bổ kỳ đã duyệt, không chứa nguyên giá TSCĐ. | `trusted` khi có ít nhất một expense hợp lệ trong kỳ; nếu chưa có thì trạng thái là `not_recorded`. |
