...
| Biến động tồn kho       | Giá trị tồn kho cuối kỳ trừ giá trị tồn kho đầu kỳ.                                | Giá vốn món, chi phí vận hành hoặc tiền đã chi cho NCC.                      |
...
| Chi phí vận hành        | Chi thuê, điện nước, gas, lương, sửa chữa, vật dụng, marketing, phí/thuế và khác. | Giá vốn món, vật tư/nguyên liệu, thanh toán NCC, nộp tiền mặt vào ngân hàng. |
| Lợi nhuận gộp           | Doanh thu thuần trừ giá vốn món/food cost.                                        | Kết quả vận hành, dòng tiền, lợi nhuận ròng.                                 |
| Kết quả vận hành        | Doanh thu thuần trừ chi phí vận hành rồi cộng biến động tồn kho đã ghi nhận.       | Lợi nhuận ròng hoặc kết quả kê khai thuế.                                   |
...
Finance landing có hai dòng công thức kết quả theo kỳ:
`Lợi nhuận gộp = Doanh thu thuần − Giá vốn món` và
`Kết quả vận hành = Doanh thu thuần − Chi phí vận hành + Biến động tồn kho`.
Số dư hiện có không theo bộ lọc; giá trị tồn kho cuối kỳ là một section riêng
theo bộ lọc; danh sách cần xử lý đặt cuối trang.
...
| `finance.expense.operating`                 | Chi phí vận hành        | Kỳ này có bao nhiêu chi phí vận hành?                                      | `expenses.amount` thuộc `rent`, `utilities`, `gas_fuel`, `salary`, `repair`, `supplies`, `marketing`, `fees_tax`, `other`; tập hợp rỗng bằng `0`. | `trusted` khi truy vấn kỳ thành công, kể cả khi không có expense hợp lệ; lỗi truy vấn là `unavailable`.                                               |
...
| `finance.operating_result`                  | Kết quả vận hành        | Sau chi phí vận hành và biến động tồn kho, doanh thu thuần trong kỳ còn bao nhiêu? | `finance.revenue.before_vat_after_discount - finance.expense.operating + (inventory.closing - inventory.opening)`.                       | Chỉ tính khi dữ liệu chi phí vận hành khả dụng; kỳ không có chi phí dùng `0`; tồn tăng làm tăng kết quả; không phụ thuộc food-cost coverage; không gọi là lợi nhuận ròng. |

### Bộ dữ liệu Finance một quán F&B cần

Finance F&B không bắt đầu bằng sổ kế toán nâng cao. Nó cần đủ dữ liệu để
...
| Giá vốn món thực tế        | Lợi nhuận gộp                                 | `stock_movements` với `type='consumption'`, `movement_subtype='sale_consumption'`.                                                                          | Đúng khi số đơn đã thanh toán có đủ `stock_movements.order_id`; thiếu coverage thì không tính lợi nhuận gộp.                                                                                                                                                                   |
...
| Chi phí vận hành           | Chi phí HKD trong kỳ                          | `expenses`, category thuộc nhóm `operating` trong `EXPENSE_CATEGORY_GROUP`.                                                                                 | Đúng khi expense đã được ghi; không tự suy ra từ PO/GRN/NCC, không gồm giá vốn món hoặc nộp tiền nội bộ.                                                                                                                                                                      |
...
| `Chi phí vận hành`           | `fetchOperatingExpenseSummary` và `/finance/expenses`.                                                               | Tổng `expenses.amount` nhóm operating: thuê, điện nước, gas, lương, sửa chữa, vật dụng, marketing, phí/thuế, khác; loại nguyên liệu, NCC và nộp nội bộ; tập hợp rỗng bằng `0`.                                                                                                                                                               | Đúng khi truy vấn kỳ thành công; không tự kéo PO/GRN/NCC vào.                                                                                                                                                                 | Không có bản ghi hợp lệ thì hiển thị `0đ`; lỗi truy vấn hiển thị `Chưa xác định`.               |
...
| `Kết quả vận hành`           | `buildKpis` trong Finance cockpit.                                                                                   | `Doanh thu thuần - Chi phí vận hành + (Tồn cuối kỳ - Tồn đầu kỳ)`.                                                                                                                                                                                                                                                                          | Chỉ hiển thị tiền khi dữ liệu chi phí vận hành khả dụng; không phụ thuộc `costAvailable`; tồn tăng làm tăng kết quả; không phải lợi nhuận ròng.                                                                            | Hiển thị `Chưa tính` khi dữ liệu chi phí không khả dụng.                                       |
