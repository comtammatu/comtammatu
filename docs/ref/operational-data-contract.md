...
| Tồn quỹ tiền mặt/ngân hàng | Dòng tiền đang giữ                       | `system_settings` opening balance/date + revenue cash + paid cash expenses + cash `supplier_payments` + signed SePay movements. Nộp tiền mặt vào ngân hàng chỉ được ghi qua webhook SePay thật và `record_sepay_cash_deposit_as_system`. | Đúng sau khi chủ quán đặt mốc tồn quỹ; trước đó chỉ hiển thị trạng thái chưa đặt mốc.                                                                                |
| Đối soát ngân hàng         | Tiền vào/ra đã gắn đúng nguồn chưa       | `webhook_events` SePay qua `fetchSepayBankTransactions` và bucket `finance.cash_reconciliation.bank_webhook`.        | Supporting workflow `needs_review`; không phải KPI doanh thu, chỉ chỉ ra dòng cần xử lý.                                                                             |
...
| Công nợ nhà cung cấp       | NCC còn phải trả, quá hạn chưa trả       | `supplier_invoices`, `supplier_payments`, branch scope qua `goods_received_notes`.                                   | Supporting AP queue; còn phải trả = `total_amount - paid_amount`, aging theo `due_date`, không tính là chi vận hành hoặc giá vốn nếu chưa đi qua contract tương ứng. |
...
| `Công nợ NCC`             | `fetchUnpaidSupplierInvoiceRisk` và `/finance/supplier-invoices`.                                  | `sum(max(total_amount - paid_amount, 0))` cho `supplier_invoices.payment_status <> 'paid'`; drilldown nhóm theo NCC/đơn mua, aging theo `due_date`, lần trả gần nhất đọc từ `supplier_payments`.                                                                                                                                                                                                                                                                                                                                                                                                                                                             | Branch filter phải đi qua GRN; chỉ là AP risk, thanh toán NCC làm giảm dòng tiền/công nợ nhưng không tự biến thành expense trong kỳ.                                                        | Hiển thị trong exception/support workflow và màn phải trả NCC.                       |
...
| `Đối soát ngân hàng`      | `/finance/bank-transactions` -> `fetchSepayBankTransactions` -> `buildSepayReconciliationSummary`. | Danh sách giao dịch SePay hợp lệ gần nhất đang hiển thị; `matched` khi tiền vào có `payment_id`, tiền vào nộp quỹ có `bank_deposit` liên kết qua `expense_id`, tiền ra có `expenseIds`, hoặc tiền ra khớp khoản trả NCC `supplier_payments` theo ngày + mã tham chiếu + số tiền; NCC match drill sang `/finance/supplier-invoices?invoiceId=...`; `needs_review` khi tiền vào chưa gắn đơn hoặc tiền ra chưa gắn nguồn; `webhook_error` chỉ dành cho lỗi nhận/xử lý webhook thật; owner có thể gắn thủ công webhook tiền vào vào một payment VietQR đã thu khi số tiền khớp. | Supporting workflow `finance.cash_reconciliation.bank_webhook`; không dùng để sửa `Tiền đã thu` hay `Chi vận hành` tự động.                                                                 | Hiển thị bucket cần rà soát trên màn giao dịch ngân hàng.                            |
