...
| `Tiền mặt theo sổ`           | `fetchCashSummary` -> `get_finance_current_funds`.                                                                   | Opening cash bất biến + cash payment completed − hoàn cash − chi cash − trả NCC cash + `accepted_adjustment` của ca POS + tổng adjustment append-only.                                                                                                                                                                                      | Tenant-wide, không theo filter; loại phát sinh trước `effective_at`; `staff_repaid` không tạo delta vì chỉ khôi phục két về mức dự thu; opening không được sửa/xóa.                                    | Hiển thị `Đang xác minh` cho đến khi Owner ghi mốc mở sổ được xác minh.                        |
...
| `Lệch tiền ca`               | `close_pos_session`, `get_cash_variance_summary`, `get_cash_variance_action_target`, `resolve_pos_session_variance`. | Dự thu = đầu ca + payment cash completed; lệch = tiền đếm lúc đóng − dự thu. `staff_repaid` ghi đủ khoản thiếu; `accepted_adjustment` cập nhật tiền mặt theo sổ.                                                                                                                                                                            | Số đếm và lệch lúc đóng bất biến; sửa phương thức payment sẽ tính lại dự thu/lệch và mở lại resolution.                                                                                                | Exception dẫn thẳng tới `/br/[id]/pos-sessions?session=[id]`.                                  |
