...
- **Quy chuẩn UX/UI:**
  - Branch `/br/[branchId]/stock` là màn `Tồn` canonical: mở thẳng dữ liệu tồn thực, cảnh báo cần bổ sung và các action ngữ cảnh được cấp quyền (`Nhập hàng`, `Kiểm kê`, `Hao hụt`). Không biến bottom tab thành thư mục tính năng. Bộ lọc dùng URL-state để giữ ngữ cảnh khi reload hoặc quay lại từ chi tiết; `/stock/on-hand` chỉ redirect về route này.
