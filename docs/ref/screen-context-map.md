...
- **Quy chuẩn UX/UI:**
  - Bottom nav **chi nhánh** (`branch_kind=branch`) theo vai trò:
    ...
    - Chuông = unread inbox. Tab **Công cụ** (`/settings`) là cửa thật: Gọi số, Đối soát ca POS, Giới hạn bán (trang), **Báo cáo tổng hợp ngày** (Daily Summary 04:00: kết quả + món + ca POS drill + bấm ca; không Chốt ngày), Phản hồi, danh mục/báo cáo kho, thiết lập bàn/POS/KDS/máy in/âm thanh. Header `⋯` chỉ Owner → Quản trị + theme. Giới hạn bán vẫn CTA Drawer trên Hôm nay. Avatar header vẫn mở Hồ sơ.
  ...
  - **Điều hướng kép & Lưới đa cột Desktop:** Mobile (<1024px) dùng thanh đáy `OperatorBottomNav`; Desktop (≥1024px) chuyển sang thanh điều hướng bên `OperatorDesktopNav` (tự ẩn thanh đáy `hideOnDesktop`, mở rộng canvas `2xl:max-w-7xl`). Mọi màn hình con (bàn, máy in, trạm bếp, POS, đối soát ca, tổng kết ngày, đội, kho) tự động dàn lưới đáp ứng 2–3 cột (`sm:grid-cols-2`, `lg:grid-cols-3`), lịch tuần 7 cột (`lg:grid-cols-7`). Toàn bộ nút bấm và thẻ danh sách đạt chuẩn cảm ứng 48px (`min-h-12` / `size="touch"` / `size="icon-touch"`).
...
- **POS:** Chiết khấu thủ công = `pos:apply_discount`; tiền vẫn ghi cột discount
  hiện có (ADR 0013). `free_side` = N phần ăn kèm **/ phần món chính**; auto khi
...
- **Quy chuẩn UX/UI:**
  ...
  - `/br/[branchId]/stock` là **stock home** CN: 4 cửa hàng hóa (tồn / Điều chuyển / Kiểm kê / hao) **trên**, list phiếu fulfillment **dưới**. Không đặt Phân công đếm / Phiếu đếm làm cửa Kho (entry từ Đội). `/stock/transfer` store → redirect `/stock`. Pad nhận `/receive/[id]` tự mở phiên kiểm nhận khi `in_transit` (hiện danh sách đếm ngay, không splash CTA) và cho chọn nhận vào Kho hoặc Bếp. Tạo DC tại `/stock/transfer/new` (xin hàng / giao đi). Yêu cầu hàng đã gỡ.
  ...
  - Branch `/br/[branchId]/stock/stocktake/[id]/count` là `DOC-WORKFLOW` đếm số đang có: first viewport là nguyên liệu đang đếm, đơn vị ghi nhận, number pad và lưu/đi tiếp; draft và submit round giữ authority Server Action/RPC hiện có. Không tải hay hiển thị số tồn hệ thống trước khi phiên hoàn tất, và không đổi tablet thành bảng control_surface.
  ...
  - Branch `/br/[branchId]/stock/count-assignments` và `/stock/count-slips` là hai `LIST` touch-native riêng cho quản lý (route stock, **entry từ Đội**): màn phân công nhóm nguyên liệu theo nhân viên; màn phiếu đếm review từng chênh lệch rồi duyệt/yêu cầu đếm lại trong bottom sheet có action sticky. Tồn lúc gửi / thực đếm / lệch (và tồn hiện tại khi hiện) luôn cùng một thang khóa: đơn vị nhân viên đã đếm + Đơn vị chuẩn; không tách phần lẻ theo đơn vị catalog khác nhau. Khác `/stock/stocktake` (phiên Kiểm kê). Không dẫn quản lý vào phiếu đếm cá nhân của chính họ và không dùng client control_surface.
  - control_surface `/inventory/count-assignments` và `/inventory/count-slips` giữ management list desktop-responsive bằng `DataTable`; chỉnh phân công và review dòng phiếu mở trong `AppDialog` với action hiển thị rõ. Dòng review phiếu dùng cùng thang khóa đơn vị đếm + Đơn vị chuẩn như Branch. Không dùng swipe, long-press, drawer hoặc presenter Branch.
