...
### 2.0. Chọn mặt phẳng và địa điểm — `/`

- **Archetype:** `GATE/AUTH`.
- **Đối tượng sử dụng chính:** Owner và các role Branch đã đăng nhập.
- **Mục tiêu Nghiệp vụ (Why?):** Tách rõ nơi Owner theo dõi/điều khiển toàn hệ
  thống khỏi nơi nhân sự vận hành một chi nhánh.
- **Mục tiêu Người dùng (Goal):** Owner chọn Admin Dashboard hoặc đúng Branch;
  role Branch đi vào Branch được gán mà không thấy lựa chọn quản trị toàn hệ thống.
- **Thông tin hiển thị:** Owner luôn thấy card Admin Dashboard và các Branch đang
  hoạt động, kể cả chỉ có một Branch. Non-Owner không thấy Admin Dashboard.
- **KHÔNG hiển thị:** KPI, công việc trong ca, hoặc module shortcut lẫn vào bước
  chọn ngữ cảnh.
...
- **Luồng tương thích:** `/br/[branchId]/dashboard` không còn actor, dữ liệu hay
  presentation riêng; route chỉ redirect về `/br/[branchId]`.
- **Quy chuẩn UX/UI:**
  - `Nay` chỉ hiển thị các hàng chờ có số lượng lớn hơn 0, sau đó là điểm vào bán hàng/bếp/đơn hàng và lối tắt Branch-native. Không lặp lại thư mục `Đội`, `Kho`; không có link Admin Dashboard hoặc lệnh mở dashboard KPI.
  - Không đưa KPI doanh thu, chỉ số toàn chuỗi, Finance, HR, payroll, global menu
    hay tenant settings vào Branch Hub. Các chỉ số/control Owner thuộc Admin Dashboard.
...
### 2.5. Kho hàng — Admin Dashboard `/inventory` và Branch `/br/[branchId]/stock`
...
- **Quy chuẩn UX/UI:**
  ...
  - Branch `/br/[branchId]/stock/on-hand/[ingredientId]` là `DETAIL` touch-native: ưu tiên trạng thái/tồn hiện tại, vị trí tồn, chuyển động gần đây, sau đó là ngưỡng và action được cấp quyền. Nhận từ NCC mở GRN, còn `/stock/receive` chỉ dành cho phiếu chuyển nội bộ; route không tải hoặc hiển thị WAC, giá trị tồn, audit/correction, hoặc detail chrome Admin Dashboard.
  ...
  - Branch `/br/[branchId]/stock/stocktake/[id]` là `DETAIL` touch-native: phiên đang thực hiện chỉ review số đếm mù/đếm lại và action tiếp tục/chốt theo quyền; khi hoàn tất mới hiển thị hệ thống, thực đếm và chênh lệch theo từng nguyên liệu. Không đưa audit history, report CTA, WAC, giá trị tồn hoặc detail chrome Admin Dashboard vào Branch.
