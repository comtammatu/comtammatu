...
State machine enforce qua RPC `transition_tax_invoice_state(id, to_status, payload?, note?)` (định nghĩa trong `supabase/migrations/20260717151345_baseline.sql`). Mọi UPDATE status PHẢI đi qua RPC — không cho phép client UPDATE trực tiếp.
...
RPC `get_finance_dashboard_summary` (định nghĩa trong `supabase/migrations/20260717151345_baseline.sql`) cung cấp counters:
...
Toàn bộ schema + RPC HĐĐT đã gộp vào `supabase/migrations/20260717151345_baseline.sql`
