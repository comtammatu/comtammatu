...
| Luân chuyển nội bộ              | Branch không mở Kho↔Bếp hay cross-branch mới (D078). `stock_transfers` lịch sử giữ trong DB/Admin Dashboard read-only.                                                                                                       | Không dùng `stock_transfer` cho tiêu hao/xuất hủy thật |
