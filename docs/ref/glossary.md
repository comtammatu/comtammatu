...
| `gross_profit`           | Lợi nhuận gộp                 | Doanh thu thuần trừ giá vốn món. Dòng độc lập; không phải cha của kết quả kỳ.                             | `net_sales_before_vat - food_cost`.                                                              | Kết quả kinh doanh, lợi nhuận ròng, dòng tiền, tiền mặt trong két.                     | `finance.gross_profit.readonly`.                                    |
| `gross_margin`           | Biên gộp                      | Tỷ lệ lợi nhuận gộp trên doanh thu thuần.                                                                  | `gross_profit / net_sales_before_vat`.                                                           | Tỷ lệ giá vốn món, biên ròng.                                                          | `finance.gross_margin.readonly`.                                    |
| `operating_expense`      | Chi vận hành                  | Chi phí kỳ đã ghi nhận: thuê, điện nước, lương, phần mềm, marketing, sửa chữa, vật tư tiêu hao, khấu hao/phân bổ, tiếp khách và phí. | Sum posted expense trong kỳ, loại direct ingredient COGS, `capital`/`construction`/`deposit`, điều chuyển, hàng nhập mua, và nguyên giá tài sản. | Giá vốn món, Chi phí ban đầu, công nợ NCC, tiền mua TSCĐ/thiết bị chưa phân bổ, chi phí hàng. | `finance.expense.operating`.                                        |
| `inbound_transfer_value` | Chi phí hàng                  | Giá điều chuyển đã nhận vào kho chi nhánh trong kỳ. Nhánh của `Chi phí` trên kết quả kỳ; không phải chi vận hành. | Tổng `inventory_value_allocations` `event_type=transfer_in` bucket `inventory` tại chi nhánh nhận. Không gồm YC, phiếu đang chuyển, thiếu POS, hóa đơn NCC. | Chi vận hành, hàng mua NCC công ty, giá vốn món.                                   | `finance.inbound_transfer_value`.                                   |
| `period_inventory_purchases` | Chi mua hàng              | Giá trị HĐ đầu vào NCC đã xác nhận trong kỳ (chưa VAT). Nhánh của `Chi phí` khi xem cả quán. Công nợ chưa trả vẫn tính. Giao dịch ngân hàng là thanh toán, không trừ lần nữa. Không gồm điều chuyển nội bộ. | `supplier_invoices.subtotal` `document_status` `confirmed`/`adjusted` theo `invoice_date`.                | Điều chuyển nội bộ, chi vận hành, giá vốn món, nháp HĐ, phiếu thiếu POS.                    | `finance.inventory_purchases`.                                      |
| `startup_capital`        | Chi phí ban đầu               | Vốn đã bỏ ra cho quán/chi nhánh: thi công, tài sản, đặt cọc. Không phải chi vận hành tháng. | Sum `expenses.amount` `capital`+`construction`+`deposit`, không theo kỳ; gồm GTGT. Chi nhánh chỉ dòng `branch_id` khớp, không gồm `NULL`. | Chi vận hành tháng, giá vốn món, HĐ NCC, khấu hao kỳ, chi Công ty. | `finance.expense.startup_capital`.                                  |
| `capital_outlay`         | Thiết bị                      | Khoản chi mở quán ghi `expenses.category = capital` (máy, xe, nội thất, thiết bị sẵn sàng dùng). Card `/finance` Tài sản; sổ `/finance/equipment`. Không phải sổ TSCĐ hay giá trị còn lại. | Cùng nguồn `startup_capital`, chỉ nhóm `capital`. Thành phần của `Tổng giá trị`.                | Thi công, đặt cọc, chi vận hành, giá vốn món, `Giá trị thiết bị`.                                                | `finance.asset.equipment`.                                  |
| `construction_outlay`    | Thi công                      | Khoản chi mở quán ghi `expenses.category = construction` (thi công, thiết kế, hạng mục chưa nghiệm thu). Card `/finance` Chi phí ban đầu; sổ `/finance/construction`. Không vào `Tổng giá trị`. | Cùng nguồn `startup_capital`, chỉ nhóm `construction`.                                           | Tài sản/thiết bị, đặt cọc, chi vận hành.                                                           | `finance.expense.construction`.                             |
| `operational_asset_value` | Tổng giá trị                  | Tổng tiền + tồn kho (nếu có quyền) + thiết bị (`capital`). Không gồm thi công / đặt cọc / chi phí ban đầu. Không phải tổng tài sản kế toán. | `addMoney` các hạng mục đang hiện; chưa mở sổ quỹ thì không bịa số.                              | Chi phí ban đầu, TSCĐ, GTGT phải nộp.                                                      | `finance.asset.total_value`.                                |
...
| `inventory_value`        | Giá trị tồn kho               | Tiền đang nằm trong tồn kho theo snapshot location/branch.                                                 | `stock_levels.current_quantity * (stock_levels.avg_unit_cost ?? 0)`. Giá tham chiếu catalog không phải giá sổ. | Chi phí trong kỳ, tiền mua NCC, profit.                                                | `finance.inventory_value.current`, `inventory.stock_value.current`. |
| `company_wac`            | Giá vốn                       | WAC một số cho nguyên liệu mua: mọi kho stock-bearing cùng đơn giá; hết hàng vẫn giữ giá cuối.             | `sum(account.book_value) / sum(account.quantity)` rồi ghi `stock_levels.avg_unit_cost`.          | Giá vốn mẻ thành phẩm; đơn giá chứng từ GRN (có thể 0); giá tham chiếu catalog.        | Inventory stock / issue / transfer / POS.                           |
| `production_wac`         | Giá vốn mẻ                    | WAC một số / mã thành phẩm: không GRN, chỉ nấu rồi điều chuyển. Giá mẻ = giá NL vừa trừ (sau restatement HĐ NL). Lệch kho là lỗi. | Origin `production_output` / equalize mọi location cùng mã TP.                                   | Giá vốn nguyên liệu mua (`company_wac`); đơn giá chứng từ GRN.                         | Production output / FG transfer / FG on-hand.                       |
...
| `expected_cash`          | Tiền mặt dự thu               | Tiền mặt POS đáng lẽ có trong két tại lúc đóng ca theo payment hoàn tất.                                   | `opening_cash + sum(completed cash payments)` có `paid_at` trong cửa sổ mở–đóng ca (kể cả đơn mở ca trước, thu ca này). | Tiền đếm thực tế; sổ TM chi nhánh bán hàng / tổng công ty trên `/finance`.             | Cash session.                                                       |
...
| `operating_expense` (`opex`)  | chi phí vận hành         | Rent, utilities, software, repair, marketing, bank/payment fee; không gồm direct COGS nếu đã tách.                | food cost, điều chuyển, hàng nhập mua |
| `inbound_transfer_value`      | chi phí hàng             | Giá ĐC đã nhận vào kho chi nhánh trong kỳ; nhánh Chi phí, không phải opex.                                        | chi phí vận hành, hàng mua NCC        |
| `period_inventory_purchases`  | chi phí hàng mua         | HĐ đầu vào NCC đã xác nhận trong kỳ; gồm chưa thanh toán; không trừ thêm khi khớp NH.                          | điều chuyển nội bộ, opex, nháp HĐ     |
...
| `bank_transaction`           | giao dịch ngân hàng    | Một biến động tiền vào hoặc tiền ra canonical (SePay hoặc sao kê MB khi SePay chưa có lịch sử), tính đúng một lần. Màn Finance LIST `/finance/bank-transactions` nhãn UI **Giao dịch**. | Webhook delivery, payment, expense; không lẫn với quy trình đối soát |
...
| `bank_reconciliation`        | đối soát ngân hàng     | Phân loại giao dịch ngân hàng vào payment/chứng từ; gắn hoặc gỡ không đổi số dư. Hành động khớp trên màn **Giao dịch**: tiền vào tìm theo **mã đơn**; không phải tên màn. | payment split, bank movement; không gọi màn hình là «Đối soát NH» |
...
| `cycle_count`             | kiểm kê luân phiên       | Lịch một phần (không còn cửa UI).                                   | stocktake toàn kho         |
...
| waste / stocktake `reason_code` | (see `WASTE_REASON_LABELS_VI`) | Required causal catalog for waste; optional historical metadata for stocktake variance and never a stocktake completion gate (`docs/ref/inventory.md` §8). |
...
| `stocktake` vs `inventory_count_slip` | Kiểm kê is the manager session that can adjust stock; Đếm tồn is a shift-assigned slip (review only). |
