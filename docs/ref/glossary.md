...
| `entry_unit_id`         | đơn vị theo chứng từ                | PO: Nhập; GRN: Nhập hoặc Xuất; DC/xuất/hao hụt: Xuất hoặc Nhập; định mức món: mọi đơn vị ladder; BOM/LSX: Sản xuất. | text unit từ client                   |
| `base_unit`             | đơn vị tồn chuẩn                    | `is_base = true`: Xuất khi không sản xuất, Sản xuất khi có; ledger, ngưỡng và WAC lưu theo đây.              | đơn vị nhập, đơn vị đóng gói          |
| `to_base_factor`        | quy đổi về tồn chuẩn                | Hệ số snapshot `1 đơn vị chứng từ = N đơn vị tồn chuẩn`; ví dụ `1 chai = 330 ml`.                            | hệ số đảo chiều                       |
...
| `average_unit_cost`     | giá vốn bình quân gia quyền (`WAC`) | `stock_levels.avg_unit_cost` theo đơn vị tồn chuẩn; UI tồn kho chiếu mặc định sang Đơn vị xuất.                | đơn giá nhập, giá trị tồn, giá vốn món |
