...
| `operating_expense`      | Chi phí vận hành              | Chi phí kỳ đã ghi nhận: thuê, điện nước, lương, phần mềm, marketing, sửa chữa, vật tư tiêu hao, khấu hao/phân bổ, tiếp khách và phí. | Sum posted expense trong kỳ, loại direct ingredient COGS, `capital`/`deposit`, điều chuyển, hàng nhập mua, và nguyên giá tài sản. | Giá vốn món, Chi phí ban đầu, công nợ NCC, tiền mua TSCĐ/thiết bị chưa phân bổ, chi phí hàng. | `finance.expense.operating`.                                        |
| `inbound_transfer_value` | Chi phí hàng                  | Giá điều chuyển đã nhận vào kho chi nhánh trong kỳ. Nhánh của `Chi phí` trên kết quả kỳ; không phải chi phí vận hành. | Tổng `inventory_value_allocations` `event_type=transfer_in` bucket `inventory` tại chi nhánh nhận. Không gồm YC, phiếu đang chuyển, thiếu POS, hóa đơn NCC. | Chi phí vận hành, hàng mua NCC công ty, giá vốn món.                                   | `finance.inbound_transfer_value`.                                   |
| `period_inventory_purchases` | Chi phí hàng mua          | Giá trị HĐ đầu vào NCC đã xác nhận trong kỳ (chưa VAT). Nhánh của `Chi phí` khi xem cả quán. Công nợ chưa trả vẫn tính. Giao dịch ngân hàng là thanh toán, không trừ lần nữa. Không gồm điều chuyển nội bộ. | `supplier_invoices.subtotal` `document_status` `confirmed`/`adjusted` theo `invoice_date`.                | Điều chuyển nội bộ, chi phí vận hành, giá vốn món, nháp HĐ, phiếu thiếu POS.                    | `finance.inventory_purchases`.                                      |
| `startup_capital`        | Chi phí ban đầu               | Vốn đã bỏ ra cho quán/chi nhánh: thi công, máy, xe, nội thất, đặt cọc. Không phải chi phí vận hành tháng. Card KPI: `Vốn đã bỏ ra`. | Sum `expenses.amount` `capital`+`deposit`, không theo kỳ; gồm GTGT. Chi nhánh chỉ dòng `branch_id` khớp, không gồm `NULL`. | Chi phí vận hành tháng, giá vốn món, HĐ NCC, khấu hao kỳ, chi Công ty. | `finance.expense.startup_capital`.                                  |
| `capital_outlay`         | Thi công / tài sản            | Khoản chi mở quán ghi `expenses.category = capital`.                                               | Cùng nguồn `startup_capital`, chỉ nhóm `capital`.                                                | Đặt cọc, chi phí vận hành, giá vốn món.                                                | `finance.expense.startup_capital`.                                  |
