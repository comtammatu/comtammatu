...
| ------------------------------------- | ---------------------------------- | -------------------------------------------------------------------------------- |
| UI cho người dùng nội bộ              | Tiếng Việt                         | Giữ acronym quen thuộc như `POS`, `KDS`, `tenant`, `GRN`, `WAC` nếu cần ngắn gọn |
...
- acronym hoặc thuật ngữ chuyên ngành đã chốt: `POS`, `KDS`, `tenant`, `ERP`, `PO`, `GRN`, `WAC`, `PIT`
...
| `site`               | site vận hành         | Specs / technical docs khi cần gom `branch`                              | dùng thay cho `branch` trong UI                      |
...
| `goods received note (GRN)`   | phiếu nhập kho                  | Hàng thực nhận từ NCC tại kho chi nhánh (chi nhánh) hoặc chi nhánh (chi nhánh) | receiving note, phiếu nhận                     |
...
| `finished good`               | thành phẩm                      | Hàng sản xuất tại chi nhánh hoặc giữ tồn để cấp xuống bếp chi nhánh            | món hoàn chỉnh nếu đang nói item tồn kho       |
...
| `production order`            | lệnh sản xuất                   | Lệnh tại chi nhánh                                                             | work order                                     |
...
`user_role` là compatibility claim trong JWT, derived từ `positions.code`. Vai trò mới (`warehouse_manager`, `production_manager`) được thêm khi Auth tách Kho và chi nhánh thành workstream riêng.
...
| ------------------ | ------------- | ------ | ----------- |
| `branch`           | kho chi nhánh | —      | `chi nhánh` |
| `branch`           | chi nhánh     | Bếp TT | `chi nhánh` |
...
- Dùng `branch` / `chi nhánh` làm canonical term cho mọi site vận hành cấp L1.
- `branches.branch_kind` là compatibility column và chỉ hợp lệ với giá trị `branch`.
...
- Chỉ viết `bếp` khi context đã đủ rõ; nếu không, phải ghi rõ `bếp chi nhánh`.
...
- Giữ acronym quen thuộc nếu người dùng đã nhận diện: `POS`, `KDS`, `chi nhánh`, `chi nhánh`, `GRN`, `WAC`.
...
- Khi sơ đồ cần rút gọn, ưu tiên label ngắn nhưng vẫn giữ canonical meaning, ví dụ `chi nhánh / kho chi nhánh`.

### Code, DB, và contracts

- Dùng English canonical terms cho table, type, enum, RPC, folder, và file.
- Không đổi qua lại giữa nhiều nhãn cho cùng một site vận hành — schema chốt là `branch`.
