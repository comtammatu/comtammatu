# Release `1.2.0.0` — Headline: QR Feedback Module (2026-05-08)

> **Headline feature: customer-facing QR feedback** with Telegram alerts, AI enrichment, and daily reports.
> Global app version `1.1.0.0 → 1.2.0.0` (minor — additive, no breaking changes). Tag: `1.2.0.0`.

## Why a global version bump (not a feature tag)

Cơm Tấm Má Tư is a single-app SaaS — all modules ship to the same Vercel deployment. Customers and owners see the app, not individual module versions. Prior tag was `1.1.0.0` (2026-04-15, GL auto-posting engine). This release's headline is the QR feedback module; minor security/perf/UX work that landed on `main` since then is also included implicitly. Work still in-flight on separate branches (`m4-payments-fix`, `d011-v2`) is preserved as `[Unreleased]` in CHANGELOG and is NOT in `1.2.0.0`.
...
Read-only QA pass against production (2026-05-07): findings are tracked in
`tasks/todo.md` under the Feedback module follow-up list.
