...
- B31 (HĐĐT provider seed: Viettel S-invoice credentials) — owner provides
...
- HĐĐT issue rehearsal pass: 1 invoice via Sinvoice test account (draft → issued → archived with PDF in Storage)
...
| HĐĐT provider abstraction leaks vendor specifics | Medium | Runtime keeps business logic behind `InvoiceProvider`; only `ViettelSinvoiceProvider` is active |
...
| Sinvoice test availability for rehearsal | Medium | Use Viettel test account and retry outside DB transaction |
...
10. Rotate or confirm secrets (Sinvoice / MoMo / VietQR / Upstash)
...
| HĐĐT provider outage | Medium | Viettel S-invoice retry/reconcile + manual portal fallback | W4 |
