...
- **Tax invoices (HĐĐT):** state machine `(draft → issued → archived)`; Viettel S-invoice is the only active provider; evidence retention via Storage
...
- **External:** Viettel S-invoice API; MoMo provider for refund webhook
...
- **B31 (module-specific)** — HĐĐT provider seed for green: Viettel S-invoice credentials. Owner to provide.
...
- **External:** Zalo ZNS (Post-v1.0), SpeedSMS, Resend, Viettel S-invoice for HĐĐT
...
- Outbound: Viettel S-invoice REST, VietQR API polling (Finance §7)
