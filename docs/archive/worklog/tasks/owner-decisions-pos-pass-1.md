...
> Archived 2026-05-23 from top-level `tasks/`. Historical POS decision worklog; active tracker remains `tasks/todo.md`.
> Audience: project owner (Ma Tu).
> Goal: 5 decisions in 1 sync, batched. Estimate ~25 phút.
> Output: each row gets a written answer in this file → updates `docs/archive/worklog/tasks/pos-qa-pass-1.md` "Open questions" section.
...
| D4 | HĐĐT for total=0 (comp meal) | **Conditional on MST** — MST nhập → gửi HĐĐT provider; không nhập → skip + flag `tax_invoice_status='not_required'` |
...
**Context:** `createTaxInvoice` precondition là `payment_status='paid'`. Sau khi fix `05.03`, comp meal pay 0đ vẫn `payment_status=paid`. Có nên gọi provider xuất HĐĐT cho hóa đơn 0đ?
...
**Nếu không quyết:** scenario `05.05` cross-flow với `05.03` chưa rõ expected behavior; invoice action có thể fail kỳ lạ với amount=0 nếu provider reject.
