...
> Archived 2026-05-23 from top-level `tasks/`. Historical UI audit worklog; active UI rules remain in `tasks/regressions.md`.
...
5. **`apps/web/app/br/[branchId]/kds/order-card.tsx` + `kds/hooks/use-kds-realtime.ts`** — Realtime correctness + peak-hour perf. **Chrome verified compliant 2026-05-07 step 6** (0 PageHero, 0 raw `font-heading text-2xl`, 0 `Loader2`/`animate-spin`). **All 4 KDS rules verified intact**: KDS-ORDERCARD-MEMO-PROPS (`order-card.tsx:176` `arePropsEqual` hand-written + line 253 `memo(OrderCardComponent, arePropsEqual)`), KDS-TRANSFER-TABLE-SYNC (`use-kds-realtime.ts:311-316` oldRow/newRow.table_id comparison + REPLICA IDENTITY FULL note line 297-301), REALTIME-CHANNEL-RESUBSCRIBE-ON-TOKEN-REFRESH (`_hooks/use-realtime-channel.ts:107-110` `onAuthStateChange` → resubscribe on `TOKEN_REFRESHED`/`SIGNED_IN`), POS-RESUME-MUST-REFETCH (`use-kds-realtime.ts:359` `addEventListener("visibilitychange", handleVisibility)`). **Test infra DOES exist** — `apps/web/playwright.config.ts` + 5 e2e specs in `apps/web/e2e/` including `kds-queue.spec.ts` (bump/recall workflow with chef login + `createKdsTestTicket` helper). Original freeze rationale was based on stale `docs/CODEBASE_MAP.md` "Critical Unknowns: No integration tests exist yet" claim — that doc is out of date; Playwright shipped already. **Freeze remains** on this rule cluster during ongoing maintenance: do NOT remove `arePropsEqual` from `OrderCard.memo()`; do NOT drop the `payload.old.table_id !== payload.new.table_id` filter; do NOT skip `TOKEN_REFRESHED` resubscribe; do NOT remove `visibilitychange` resume-refetch. When extending KdsOrder/KdsTicket shape with a render-affecting field, ADD it to `arePropsEqual` in the same diff (per rule). After step 6: NO surface left in chrome-rebuild freeze list — KDS rules still apply, but as ongoing maintenance discipline, not as a rebuild blocker.
