...
> Archived 2026-05-23 from top-level `tasks/`. Historical POS QA worklog; active tracker remains `tasks/todo.md`.
...
- All 5 owner decisions resolved 2026-04-26 (`docs/archive/worklog/tasks/owner-decisions-pos-pass-1.md`). P0 spec gap on close-shift: closed.
...
**Engineering pre-work** (out of QA scope but unblocks runs): D1 + D3 RPC change, D4 invoice orchestrator change — see `docs/archive/worklog/tasks/owner-decisions-pos-pass-1.md` follow-up table.
...
| VietQR / MoMo / Sinvoice | **mock/test mode** (env flag) — real banking out |
| Payment test harness | VietQR/MoMo tests MUST exercise action/webhook path. Direct DB update fallback is invalid. |
| Skip on dev DB | real Sinvoice push, SMS receipt, printer hardware |
...
Full context in `docs/archive/worklog/tasks/owner-decisions-pos-pass-1.md`. TL;DR:
