...
`kpi-card`, `label`, `note-callout`, `popover`, `progress`, `radio-group`, `scroll-area`,
...
| ------------------------------------------------ | ------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `/stock`                                         | LANDING       | Stock home: fulfillment slips + four doors (`Kho hàng` / `Điều chuyển` / `Kiểm kê` / `Hao hụt`). Store `/stock/transfer` redirects here.                             |
...
| `/stock/grn`                                     | LIST          | Central only (`loadGrnListPageData`). Store kind redirects to `/stock/transfer`.                                                                                    |
| `/stock/grn/new`, `/stock/grn/new/[supplierId]`  | REDIRECT-SHIM | Store → pull transfer create (`/stock/requests/new`); `Kho Tổng`/`Bếp TT` → `Tạo đơn`. No live GRN create UI.                                                         |
...
| `/stock/transfer`                                | LIST          | Store redirects to `/stock`. Central keeps the fulfillment queue. Create is `/stock/transfer/new`.                                                                   |
