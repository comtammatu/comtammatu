...
- `LinkCardGrid` cho grid responsive (1/2/3/4 cột) bọc các `AppLinkCard`.
...
- Hệ thống xác thực chỉ có hai mặt phẳng. Admin Dashboard chỉ dành cho Owner,
  chứa KPI/điều khiển/thiết lập toàn hệ thống tại `/admin`, `/inventory`,
  `/finance`, `/hr`, `/menu`, `/orders`, `/branches` và `/admin/settings`.
  Branch là mặt phẳng làm việc hằng ngày của Branch Manager và Staff tại
  `/br/[branchId]/*`; Owner có thể vào cùng mặt phẳng để giám sát hoặc hỗ trợ.
- data loader, Server Action, RPC và permission check có thể dùng chung giữa
  hai plane; presentation không được dùng chung nếu Admin Dashboard component tạo cảm
  giác desktop thu nhỏ trong Branch. Khi cần tách, Branch route dùng native
  `BranchOperator*` component và giữ Admin Dashboard route cho oversight/dense table.
- POS, KDS, Runner là các chế độ chrome toàn màn hình bên trong Branch; chúng
  không tạo thêm product plane, không bọc vào Branch Hub bottom-nav và không
  dùng Admin Dashboard shell.
- `/notifications` là tiện ích chrome-less của Branch cho mọi vai trò đã đăng
  nhập. Self-order và các route public nằm ngoài hai mặt phẳng đã xác thực.
...
- hub là home duy nhất của Branch: trạng thái ca, ngoại lệ/hàng chờ có số lượng
  lớn hơn `0`, rồi một menu công cụ theo quyền. Không đặt `Điều hành chi nhánh`
  hoặc `Cài đặt chi nhánh` như tile trong Hub.
- `/br/[branchId]/dashboard` chỉ là `REDIRECT-SHIM` về hub;
  `/br/[branchId]/settings` là workflow quản lý trong cùng mặt phẳng Branch,
  không phải một mặt phẳng hay shell quản lý riêng.
- operator/operations/employee-lib không import hoặc render Admin Dashboard
  chrome; guard `operator-admin-dashboard-shell-boundary` bắt mọi đường tắt qua
  `AppShell`, `AdminDashboardModuleShell`, `FinanceShell`, hoặc `InventoryShell`.
...
- `/br/[branchId]/shift/leave-approvals` là Branch-native touch `LIST`: tab
  ...
  trong bottom `Sheet` có action sticky. Admin Dashboard giữ `LeaveRequestsTable`;
  Branch không import bảng hoặc page presenter HR Admin Dashboard.

Branch stock workflow áp dụng cùng ranh giới này:

- `/br/[branchId]/stock` là màn `Tồn` canonical: mở thẳng dữ liệu tồn thực và
  các action ngữ cảnh theo quyền, không là feature hub và không wrapper Admin Dashboard
...
- `/br/[branchId]/stock` là Branch-native touch `LIST`: dùng shared
  `loadStockOnHandPageData` + pure filter model nhưng giữ `ItemGroup` touch với
  grid một cột trên phone, hai cột từ tablet portrait và ba cột từ tablet
  landscape. Mỗi item vẫn là một vùng chạm đầy đủ; route không đổi sang
  `DataTable` tại `1024px`, không hiển thị WAC/giá trị tồn/KPI, và mở thẻ kho
  qua `/br/[branchId]/stock/on-hand/[ingredientId]`.
...
- `/br/[branchId]/stock/on-hand/[ingredientId]` là Branch-native touch `DETAIL`:
  ...
  hàng chuyển nội bộ. Không đưa WAC, giá trị tồn, audit/correction, Admin Dashboard
  `AppPageHeader`, `DataTable`, hoặc Admin Dashboard detail presenter vào route này.
- `/br/[branchId]/stock/grn` là Branch-native touch `LIST`: dùng shared
  ...
  Admin Dashboard presentation tại tablet landscape.
- `/br/[branchId]/stock/grn/new` là Branch-native touch `LIST` cho bước chọn
  ...
  source không render `DocumentFormFrame`, `DataTable`, hoặc Admin Dashboard picker.
...
- `/br/[branchId]/stock/stocktake` là Branch-native touch `LIST`: session
  ...
  Admin Dashboard toolbar, branch picker, audit, hay report CTA.
- `/br/[branchId]/stock/stocktake/new` là Branch-native `DOC-WORKFLOW`: URL
  ...
  `AppDetailFooter` sticky; không import `DocumentFormFrame` hoặc Admin Dashboard start
  presenter.
...
- `/br/[branchId]/stock/stocktake/[id]` là Branch-native touch `DETAIL`: active
  ...
  variance. Không đưa audit history, Admin Dashboard detail client, report CTA, WAC, hay
...
- `/br/[branchId]/stock/issues/[id]` là Branch-native touch `DETAIL`: nháp cho
  ...
  giá trị, audit history, `DocumentStockCorrectionDialog`, Admin Dashboard detail client,
...
- Assignment nhóm theo nhân viên thuộc tab Đội tại
  ...
  `routeBranchId`/`embedded` từ Admin Dashboard clients.
...
- Admin Dashboard `/inventory/stock` dùng cùng loader/model nhưng giữ management
  `StockClient`: compact card một cột trên phone, hai cột từ tablet đến
  `1279px`, và dense `DataTable` từ `1280px`. Tín hiệu chỉ đọc nằm trong page
  header; toolbar chỉ giữ tìm kiếm, filter và action. Admin Dashboard client không có
  `embedded` mode hoặc Branch route branching.
...
- detail điều chuyển trong Branch chỉ giữ thao tác giao/nhận và số lượng từng
  dòng; audit history và correction sau khi chốt thuộc Admin Dashboard management.
- `/br/[branchId]/stock/transfer/new` là redirect tương thích về `Tồn`. Tạo mới
  và lịch sử điều chuyển thuộc Admin Dashboard; Branch chỉ giữ nhận phiếu đang dở để hoàn
  tất dữ liệu cũ.
...
- Branch route không link/redirect/revalidate vào Admin Dashboard roots cho cùng job của
  branch role; cầu nối Admin Dashboard chỉ là explicit owner context, không là tile vận
  hành mặc định.

## Admin Dashboard Shell Structure

Admin Dashboard chrome (`apps/web/app/components/app-shell.tsx`) chỉ dành cho
Owner và render một sidebar trong một `SidebarProvider`:

- Tab chính = mô-đun cross-module, single-sourced bởi `resolveAdminDashboardPrimaryTabs`.
...
trải phẳng mọi page con thành tab chính; deep nav nằm trong sub-tab của module
đang active. Điều hướng Branch không được chèn vào Admin Dashboard. Dưới `lg`,
bottom-nav ưu tiên `tier2` và tab `Mô-đun` mở drawer đầy đủ; từ `lg` trở lên,
bottom-nav ẩn và Admin Dashboard dùng sidebar cố định.
...
bottom-nav, hoặc inline `ShellNavGroup[]`; Admin Dashboard dùng `AppShell`,
`AdminDashboardModuleShell` và `AdminDashboardBottomNav`, còn Branch dùng
`AppBottomNav` qua chrome đã duyệt cùng các nav resolver hiện có.
