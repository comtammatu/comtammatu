...
| Shift roster (hard clock-in gate)     | Owner: `/hr/attendance?tab=roster` (all sites + Văn phòng); BM: `/br/[branchId]/shift/roster` | `hr:assign_shift` (Owner tenant-wide; BM branch-scoped)                                                 | `reconcileShiftAssignmentsWeek`, `copyShiftAssignmentsWeek`; clock-in via `self_service_clock_in` requires assignment                      | One assignment per employee/day; no wall-clock shift fallback; writes only through SECURITY DEFINER RPCs |
