...
(e) **VAT — accountant-gated.** Theo `einvoice-tax.md §2`: HKD nộp % trên doanh thu, ăn uống **GTGT 3% gốc / 2,4% giảm tạm (đến 31/12/2026) / 0 nếu doanh thu ≤ 1 tỷ/năm**; **8% là phương pháp khấu trừ — SAI cho Má Tư.** `vat_rate` lưu **điểm %** (3.00, không phải 0.03). ⇒ đổi `SYSTEM_SETTING_DEFAULTS.vat_rate` khỏi `8` + guard chặn set 8; **giá trị cuối (3.00/2.40/0) chờ kế toán xác nhận** Má Tư đã vượt 1 tỷ chưa (skill tax-vn cấm agent tự chốt số). KHÔNG sửa số tới khi owner/kế toán trả lời.
