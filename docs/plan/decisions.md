...
1. **Hai flag, hai switch UI** (`/br/{id}/settings/pos`): `pos_stock_outcome_posting` CHỈ posting (trừ kho khi paid + first-ready, không bao giờ chặn bán); flag mới `pos_stock_availability_gate` CHỈ gate availability, default OFF, chỉ hiệu lực khi posting bật (`GATE_eff = GATE AND DED`). Rollback ladder: tắt GATE trước, DED sau (sửa D053 §1).
...
4. **Stock gate Phase 1 = advisory** (RPC availability + POS client), KHÔNG thêm hard trigger mới trên `order_items`; manual lane giữ nguyên trigger hiện có. Xét lại chỉ khi có incident oversell thật.
...
8. **Khóa Path 2 trước khi re-enable posting:** REVOKE `transition_order_status` khỏi `authenticated` + mở rộng idempotency `post_pos_sale_consumption_if_ready` sang mọi row `type='consumption'`. Sửa D016 consequences: shortage lúc outcome → post âm + alert `high`, KHÔNG fail webhook/payment; số phận `payments.stock_consumed_status` (viết hoặc xóa reader) chốt ở M4.
