...
7. **Chờ owner chốt (không chặn thi công trên):** (a) role template `production_manager`/head_chef đang thiếu key `grn_create/grn_confirm`, `stocktake_create/complete`, `writeoff` — tile Bếp TT render nhưng thao tác sẽ bị RLS/RPC chặn đến khi owner cấp key (migration nhỏ, lát riêng); (b) có bỏ nhóm "Văn phòng" (office_bridge) ở home site trung tâm không — hiện giữ nguyên; (c) bật grid tồn kho cho plane office hay không — hiện chỉ embedded.
