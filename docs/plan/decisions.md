...
4. PO và GRN không có giá do Kho nhập. GRN chỉ dùng giá Hóa đơn NCC đã xác nhận
   gần nhất của cùng NCC–nguyên liệu–đơn vị làm giá tạm; không dùng WAC hoặc giá
   tham chiếu. Khi chưa có giá Hóa đơn NCC đã xác nhận, GRN ở `cost_pending` với
   giá trị tạm bằng `0`.
...
4. Kho không có Cost Close hoặc kỳ giá vốn riêng. Late invoice tiếp tục quyết
   toán theo ngày xác nhận trong valuation subledger; việc khóa kỳ kế toán, nếu
   có, là authority riêng của Accounting.
