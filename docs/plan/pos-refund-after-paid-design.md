...
Trạng thái: **Phase 1 ĐÃ VÀO REPO** — Q0–Q9 chốt (D049); migration thật + ACL TS + decision. Owner apply migration → `pnpm db:types` → backfill, rồi Phase 2 (Server Action/UI/test). Kế toán còn phải xác nhận mốc kê khai cho cross-period (Q3).
...
## ✅ Quyết định đã CHỐT (owner — xem D049)

Owner đã chốt toàn bộ Q0–Q9 (`docs/plan/decisions.md` D049, giới hạn D023). Migration
thật: `supabase/migrations/20260628120000_pos_refund_void_after_paid.sql`.

| # | Ai quyết | Câu hỏi | Chốt |
|---|---|---|---|
| **Q0** | **Owner** | Đảo D023 (cho phép hủy/hoàn + huỷ HĐĐT ở màn POS, manager-gated)? | **RESOLVED** — đảo D023 ở phạm vi HẸP: chỉ **full void-after-paid** mở ra POS; partial / điều chỉnh / thay thế + daily_summary GIỮ owner+kế toán. Ghi ở D049. |
