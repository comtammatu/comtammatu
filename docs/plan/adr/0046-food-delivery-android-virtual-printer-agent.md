...
ShopeeFood emits ESC/POS jobs to a network printer and does not share a public order API. The intake device must not depend on a SUNMI terminal or its built-in print service. An ordinary Android phone can expose a raw TCP printer endpoint on loopback or, by explicit opt-in, the branch LAN.

The previous implementation bound its local endpoint through Android's generic loopback lookup, which resolves to IPv6 on Android, while operator instructions configured delivery apps for IPv4 `127.0.0.1`. It also labeled every captured receipt as ShopeeFood and forwarded bytes to a SUNMI-only AIDL service. Later Agent builds also retained unused detectors for other delivery apps; those apps are not a direct Agent intake path.
...
4. The Agent classifies ShopeeFood from receipt signatures after ESC/POS text extraction or on-device OCR. Missing ShopeeFood evidence fails closed: the raw receipt is retained locally as `UNCLASSIFIED` and is not posted. Other delivery platforms are out of Agent scope and stay on POS manual entry.
...
- Platform attribution is explicit: only a ShopeeFood signature may create a POS order through this Agent.
...
- Device acceptance verifies ShopeeFood against `127.0.0.1:9100` on an ordinary Android phone.
