...
**Phạm vi:** Branch operator Inventory (`/br/[branchId]/stock/**`) và pattern embed từ Admin Dashboard Inventory.\
...
Inventory operator **chưa tối ưu kiểu “một viewport + ScrollArea + Table body”** không phải vì thiếu primitive, mà vì **contract hiện tại cố ý dùng document-scroll** (cuộn cả trang trong shell `h-dvh`) và hầu hết màn stock chỉ là **EMBED-WRAPPER** nhúng Admin Dashboard Inventory.
...
| Catalog list           | categories / ingredients / units / …                    | Embed settings Admin Dashboard + `DataTable` — document-scroll                                                                                         |
...
| DOC tạo/sửa            | GRN new, PO new, transfer new, waste, production new    | `DocumentFormFrame` (Admin Dashboard) hoặc bare flex khi `embedded` + `AppDetailFooter sticky` — **body vẫn flow/document-scroll**, không `ScrollArea` |
...
3. **Lịch sử kiến trúc:** một `PageContent` phục vụ Admin Dashboard + Branch (`embedded`). Admin Dashboard là desktop management (cuộn dài ổn); Branch PWA kế thừa cùng body → cảm giác “chưa tối ưu viewport” trên điện thoại.
...
### Phase 3 — Catalog, detail và Admin Dashboard parity
...
- **EMBED + Admin Dashboard chung code:** đổi shell phải qua nhánh `embedded`, không phá Admin Dashboard `xwide`.
...
Cảm giác thiếu ScrollArea/Table “khóa viewport” là đúng với trải nghiệm PWA, nhưng **không phải bug so với rule hiện tại** — Inventory đang theo **document workflow + embed Admin Dashboard**.
