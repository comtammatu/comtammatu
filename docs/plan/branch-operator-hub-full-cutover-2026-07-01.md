...
| `/br/[branchId]/settings`              | Branch setup hub       | Configure this branch's floor setup             | `branch_settings` / `operator-bottom-nav`     | Links to tables, POS terminals, printers, KDS stations                                                                                                                        | No HR/payroll/finance/procurement/reconciliation links                                              |
...
| `/br/[branchId]/menu-limits`           | Branch sellability     | Set `Tồn`, `Sẵn bán`, `Còn` for branch items    | `branch_menu_limits` / `operator-bottom-nav`  | `fetchBranchMenuDailyLimits`, `setBranchMenuDailyLimit`, `clearBranchMenuDailyLimit`                                                                                          | Branch operation surface, not global menu editor                                                    |
| `/br/[branchId]/pos-sessions`          | Branch end-day review  | Review POS sessions for this branch             | `branch_pos_sessions` / `operator-bottom-nav` | `getPosSessionReport`, `resolvePosSessionVariance`                                                                                                                            | Branch operation reconciliation, not branch setup or finance dashboard                              |
...
- `/br/[branchId]/settings` must stay setup-only: tables, POS terminals, branch
  stock block, printers, KDS stations.
...
- `/employee` staying home for `office` (and for `warehouse_manager`/`production_manager` until D055 §1 ships) is INTENTIONAL, not a leftover — proxy redirects only fire for roles with `operator_home`. Do not delete `/employee` while any role homes there. Direction for the central-site roles: `docs/plan/decisions.md` → D055.
- POS/KDS stations keep the return-to-Hub link only inside real PWA banners (offline, update, install). Quiet mode renders no extra toolbar row so the station screen stays task-first. Runner remains excluded as a guest display.
