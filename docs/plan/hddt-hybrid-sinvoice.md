# HĐĐT Hybrid via Viettel S-invoice — Plan
...
> **Pre-req:** Owner đăng ký template "HĐ tổng hợp B2C" với CQT qua Viettel S-invoice / Viettel BU portal (D7, leadtime 3-7 ngày, song song với coding).
> **Provider correction 2026-05-09:** Cơm Tấm Má Tư sử dụng Viettel S-invoice / Sinvoice cho production HĐĐT, không phải MISA. Canonical plan file is `docs/plan/hddt-hybrid-sinvoice.md`; the old `hddt-hybrid-misa.md` path is only a legacy redirect.
> **Carryover blocker (resolved):** Provider Viettel Sinvoice đã có (`packages/shared/src/providers/impl/viettel-sinvoice.ts:115-426`). MISA chỉ còn là provider legacy/optional khi env explicit chọn `INVOICE_PROVIDER=misa`.

## Goal

Triển khai HĐĐT hybrid qua **Viettel S-invoice / Sinvoice** cho mô hình F&B CTCP:
...
| D3 | Refund-after-batch: manual qua Viettel S-invoice portal cho pilot (kế toán xử lý ngoài hệ thống) | Pilot ~30-50 refund/tháng; HĐ điều chỉnh tự động defer đến P1 |
...
| D7 | Owner đăng ký template "HĐ tổng hợp B2C" với CQT qua Viettel S-invoice / Viettel BU portal | Pre-req pháp lý, owner action item ngoài code |
...
- Deterministic `transactionUuid = sha256(tenant_id || order_id || attempt_seq)` persist trước provider call
...
- RPC `aggregate_daily_b2c_invoice(p_branch_id, p_summary_date, p_actor)` — chỉ lock + insert draft + junction (KHÔNG gọi provider HTTP)
...
**Shared:**
- Reuse `ViettelSinvoiceProvider` (`packages/shared/src/providers/impl/viettel-sinvoice.ts`) — provider canonical cho production
...
| 2 | HĐ điều chỉnh giảm cho refund-after-batch | Pilot manual qua Viettel S-invoice portal |
| 3 | Provider config encrypted DB | Env-only đủ cho single-tenant CTCP |
| 4 | PDF/XML download UI cho khách | Link Viettel S-invoice portal đủ pilot |
...
| 8 | Webhook provider inbound (push status) | Polling qua manual retry đủ pilot |
...
BEGIN
  ...
  -- Note: field name is legacy from early MISA plan; payload is provider-neutral.
  ...
END;
$$;
...
export async function POST(request: Request) {
  ...
  // 3. ensureInvoiceProviderRegistered()  // provider needs env injection
...
- `createTaxInvoice` (lines 58-337): refactor sang state machine
  ...
  - Call Viettel S-invoice via provider
...
3. `HDDT-BATCH-RESERVE-DRAFT-BEFORE-PROVIDER-CALL` — RPC inserts draft + COMMIT BEFORE caller calls provider
4. `HDDT-BATCH-NO-MISA-CALL-INSIDE-RPC` — historical rule name; means no provider HTTP call in aggregation RPC body
...
- **Provider:** Viettel Sinvoice (`packages/shared/src/providers/impl/viettel-sinvoice.ts`) là canonical production provider. MISA tồn tại như legacy/optional provider — switch qua env `INVOICE_PROVIDER=viettel|sinvoice|misa`.
...
- `packages/shared/src/providers/impl/viettel-sinvoice.ts:115-426` — Viettel S-invoice provider (canonical production target)
- `packages/shared/src/providers/impl/misa.ts:75-187` — MisaProvider (legacy/optional)
...
- Webhook provider inbound (replace polling)
- Multi-tenant per-tenant S-invoice/MISA provider Map (when multi-tenant materializes)
