...
- ✅ **matu-dev credentials — RESOLVED 2026-05-30.** Owner confirmed `SUPABASE_PASSWORD` in `.env.local` is the matu-dev DB password. `scripts/supabase-baseline-extract.mjs` now has a `KNOWN_TARGETS` allowlist (iexws + matu-dev): pass `--project-ref=nikkridjukdbqvkvqlmi` and it reads `SUPABASE_PASSWORD` + ref-swaps the shared-region session-pooler host internally (password never on the command line; logs redacted; always `--db-url`, never `--linked`). Dry-run validated — produced the matu-dev public-schema dump plan.
- 🔴 **Docker running — THE SINGLE REMAINING BLOCKER, needed for BOTH extract AND replay.** The Supabase CLI runs `pg_dump` inside a pinned Docker image even for a remote `--db-url` dump (verified 2026-05-30: `db:baseline:extract` failed with "Cannot connect to the Docker daemon"), and `db:baseline:local-check` separately needs `supabase db start`. **Start Docker Desktop, then Steps 1–2 run end-to-end** — both are read-only against matu-dev + a scratch local DB (no rebuild). Step 4 (rebuild) is the only destructive step and stays owner-gated.
- (Alternative if Docker stays unavailable: install a matching `pg_dump` on PATH — but the CLI-in-Docker path is the supported one and avoids version mismatch.)
