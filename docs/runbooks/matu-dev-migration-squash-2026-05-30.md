...
- ✅ **Docker NOT required — EXTRACT DONE Docker-free 2026-05-30.** The Supabase CLI wraps pg_dump in Docker, but the already-installed `libpq` pg_dump (18.3, at `$(brew --prefix libpq)/bin/pg_dump`) dumps matu-dev directly through the session pooler — no Docker. `pg_dump --schema-only --schema=public --no-owner` (PGPASSWORD from `.env.local` `SUPABASE_PASSWORD`, never on the command line) produced a faithful **118-table** artifact (1.46 MB) that reflects the Phase-1-cleaned state (dropped policies `sti_*`/`pp_*`/`attendance_*self*` absent; attendance INSERT-revoke present — authenticated/anon grants have no INSERT). Object counts match the manifest (118 tables / 279 fns / 273 public policies; 329 `CREATE INDEX` + the rest as PK/UNIQUE constraints). MINIMAL managed-surface noise: **0** publications / event triggers / extensions (excluded by `--schema=public`), **0** `OWNER TO` (via `--no-owner`). Only 2 trivial fixups needed for fresh-Supabase replay: `CREATE SCHEMA public;` → `CREATE SCHEMA IF NOT EXISTS public;`, and strip the 2 psql `\restrict`/`\unrestrict` meta-lines (pg_dump 18). → **Toolchain — DONE (commit `f92cc4d4`):** the Docker-free libpq pg_dump engine is now the DEFAULT in `scripts/supabase-baseline-extract.mjs` and bakes in both fixups; `--engine=cli` keeps the Docker path. `pnpm db:baseline:extract -- --project-ref=nikkridjukdbqvkvqlmi` reproducibly rebuilds the baseline from matu-dev, no Docker.
- ✅ **Replay confirmation — PASSED 2026-05-30 (owner chose rebuild matu-dev).** Built `/tmp/matu-dev-rebuild.sql` = `DROP SCHEMA public CASCADE` + the baseline (`\restrict` lines commented) + realtime publication re-add (11 tables, excluded by `--schema=public`) + `NOTIFY pgrst`. Applied via `psql --single-transaction -v ON_ERROR_STOP=1` (atomic — any error rolls back, matu-dev stays intact; original dump kept as restore artifact). Result: **exit 0, no errors**. Verified matu-dev rebuilt EXACTLY: 118 tables / 279 fns / 273 public policies / 523 indexes / 6 matviews / 11 realtime / auth hook present / Phase-1 policy drops absent / attendance INSERT-revoke intact. `SUPABASE_PROJECT_ID=nikkridjukdbqvkvqlmi pnpm db:types` from the rebuilt DB == committed types (no diff); `pnpm typecheck` 6/6. → **The consolidated baseline boots from an empty public schema — the exact property the 378-file chain lacks. matu-dev now runs on the single baseline.** (Also confirms matu-dev == the greenfield-cut schema: no `legacy_role_code`.)
- ✅ **Managed-surfaces companion — DONE 2026-05-30:** `supabase/managed-surfaces.install.sql` (extensions / 5 buckets / 14 `storage.objects` policies / realtime / 10 cron jobs), generated from matu-dev + iexws, idempotent. Apply AFTER the public baseline on a fresh env. ⚠️ **Collateral found + fixed:** the rebuild's `DROP SCHEMA public CASCADE` ALSO dropped the 11 `storage.objects` policies that reference public functions (`has_permission`, `auth_tenant_id`) — matu-dev had silently fallen from 14 → 3 policies (my earlier "verified exact" covered public objects only, not storage). Restored all 14 on matu-dev via the companion's policy section (MCP could create them — ISSUE-004's owner-block did not apply here); verified `storage.objects` back to 14. Lesson: dropping/rebuilding `public` cascades to storage policies → always re-apply the companion's storage section after.
- ✅ **Footgun FIXED (commit `8f73885f`):** `scripts/gen-types.mjs` now resolves the project from `$SUPABASE_PROJECT_ID` → `.env.local` `SUPABASE_PROJECT_ID` (= matu-dev, the cut dev source) → hardcoded fallback. A bare `pnpm db:types` now yields the committed cut types (no diff), instead of silently pulling prod's uncut schema (which had reintroduced `legacy_role_code`).
