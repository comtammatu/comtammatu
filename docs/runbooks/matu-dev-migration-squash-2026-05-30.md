...
- ⚠️ **Toolchain footgun:** bare `pnpm db:types` defaults to iexws PROD (`DEV_PROJECT_ID` fallback in `scripts/gen-types.mjs`) when `SUPABASE_PROJECT_ID` is not exported in the shell — that pulls prod's UNCUT schema and reintroduces `legacy_role_code`. Always run `SUPABASE_PROJECT_ID=nikkridjukdbqvkvqlmi pnpm db:types` for the cut/dev schema (or fix the default).
