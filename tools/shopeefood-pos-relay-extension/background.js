...
const RELAY_TAB_URL = 'https://partner.shopee.vn/';
const RELAY_TAB_QUERY = {
  url: [
    'https://partner.shopee.vn/*',
    'https://partner.shopeefood.vn/*',
    'https://merchant.shopeefood.vn/*',
    'https://*.foodypos.com/*',
  ],
};
...
async function ensureRelayTab() {
  try {
    ...
    if (tabs.length === 0) {
      console.log('[Shopee POS Relay] No merchant tab found, opening one...');
    ...
    for (const tab of tabs) {
      ...
      if (tab.discarded || tab.status === 'crashed') {
        console.log(`[Shopee POS Relay] Relay tab ${tab.id} is ${tab.discarded ? 'discarded' : 'crashed'}, reloading...`);
        await chrome.tabs.reload(tab.id);
      }
    }
  } catch (err) {
    console.warn('[Shopee POS Relay] Tab watchdog check failed:', err);
  }
}
