/**
 * Client-safe VietQR EMVCo/NAPAS payload helpers.
 *
 * Keep this file free of Node-only imports so POS Client Components can
 * generate transfer QR codes locally without calling img.vietqr.io.
 */
...
export function sanitizeVietQrText(value: string, maxLength: number): string {
  return value
    .normalize("NFD")
    .replace(/[\u0300-\u036f]/g, "")
    .replace(/đ/g, "d")
...
export function buildVietQrEmvco(input: {
  ...
  const description = sanitizeVietQrText(input.description ?? "", 25);
  const merchantName =
    sanitizeVietQrText(input.accountName ?? "", 25) || "MERCHANT";
