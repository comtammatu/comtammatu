...
function sourceBetween(source: string, start: string, end: string): string {
  ...
  const endIndex = sqlIndexOf(source, end, startIndex);
...
test("SePay webhook only settles payment; the HĐĐT worker owns issuance", () => {
  ...
  assertSqlNotMatch(src, /issueTaxInvoiceForPaidOrder|createInvoice/);
  assertSqlNotMatch(src, /confirm_sepay_payment/);
  assertSqlMatch(src, /"reconcile_sepay_order_evidence"/);
...
test("Finance requeues only the exact blocked HĐĐT job", () => {
  ...
  assertSqlMatch(actionSrc, /requeue_tax_invoice_issue_job/);
  assertSqlMatch(actionSrc, /requeue_invoice_total_mismatch_jobs/);
  assertSqlNotMatch(actionSrc, /reissueAllDraftInvoices/);
...
test("per-order HĐĐT payload expands POS modifiers and sides", () => {
  ...
  assertSqlMatch(projectionMigration,
  ...
  assertSqlMatch(snapshotServiceChargeMigration,
  ...
  assertSqlNotMatch(lineItemsSrc,
    /Phí dịch vụ/,
    "HĐĐT provider lines must not emit a named service-charge row",
  );

  assertSqlMatch(snapshotMigration, "v_old.invoice_snapshot - 'submissionSnapshot'", "replacement must copy the original immutable financial snapshot",
  );
  assert.ok(
...
test("order lines snapshot item VAT without annual-revenue inference", () => {
  ...
  assertSqlMatch(providerInitSrc,
    /\/\^1\\\//,
...
test("invoice profile and replacement cutover stay fail-closed", () => {
  ...
  assertSqlMatch(migration, /CREATE TABLE public\.invoice_profiles/);
  assertSqlMatch(migration, /'1\/001'/);
  assertSqlMatch(migration, /'C26TCS'/);
  assertSqlMatch(migration,
    /FROM public\.tenants tenant[\s\S]*ON CONFLICT \(tenant_id, version\) DO NOTHING/,
  );
  assertSqlMatch(migration, /template_code ~ '\^1\/'/);
  assertSqlMatch(migration,
  ...
  assertSqlMatch(migration, /ALTER COLUMN vat_rate DROP DEFAULT/);
  assertSqlMatch(migration, /CHECK \(vat_rate IN \(0, 5, 8, 10\)\)/);
  assertSqlMatch(migration, /'vat_rate', item\.vat_rate/);
  assertSqlMatch(migration, /'version', 1/);
  assertSqlNotMatch(migration, /'version', 2/);
  assertSqlMatch(migration, /submission_snapshot/);
  assertSqlMatch(migration, /Replacement confirmed by provider/);
  assertSqlMatch(migration,
...
test("POS item-level discount migration and actions exist", () => {
  ...
  assertSqlMatch(vndOnlyMigration,
