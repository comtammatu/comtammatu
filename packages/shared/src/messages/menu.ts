// Menu-management Vietnamese copy (extracted from menu JSX).
export const MENU_VI = {
  title: "Thực đơn",
  eyebrow: "Danh mục kinh doanh",
  itemsTab: "Món ăn",
  importExport: "Nhập / Xuất",
  importMenuItem: "Nhập từ tệp",
  exportXlsxFull: "Xuất .xlsx (đầy đủ)",
  exportCsvItems: "Xuất .csv (món ăn)",
  importSuccess: "Nhập dữ liệu thành công",
  importDialogTitle: "Nhập thực đơn từ tệp",
  itemDetailTitle: (name: string) => `${name} — Chi tiết`,
  loadItemsFailed: "Không thể tải món ăn",
  loadDataFailed: "Không thể tải dữ liệu thực đơn.",
  loadDataFailedRetry: "Không thể tải dữ liệu. Vui lòng thử lại.",
  variantsSaved: "Đã lưu biến thể",
  modifiersSaved: "Đã lưu tùy chọn",
  variantNamePlaceholder: "Ví dụ: Phần lớn",
  priceDeltaLabel: "+/- Giá",
  removeVariant: "Xóa biến thể",
  addVariant: "Thêm biến thể",
  saveVariants: "Lưu biến thể",
  modifierNamePlaceholder: "Ví dụ: Thêm trứng",
  removeModifier: "Xóa tùy chọn",
  addModifier: "Thêm tùy chọn",
  saveModifiers: "Lưu tùy chọn",
  sidesEmptyTitle: "Chưa có món phụ nào",
  sidesEmptyDescription: 'Tạo danh mục loại "Món phụ" và thêm món trước.',
  sidesSelectHint: "Chọn các món phụ có thể đi kèm:",
  sideDefault: "Mặc định",
  saveSides: "Lưu món kèm",
  variantsTab: "Biến thể",
  modifiersTab: "Tùy chọn",
  sidesTab: "Món kèm",
  itemNameRequired: "Tên món không được trống",
  itemNameMax: "Tên món tối đa 100 ký tự",
  categoryRequired: "Vui lòng chọn danh mục",
  priceRequired: "Giá không được trống",
  priceInvalid: "Giá không hợp lệ",
  descriptionMax: "Mô tả tối đa 500 ký tự",
  editItemTitle: "Chỉnh sửa món",
  addItemTitle: "Thêm món mới",
  itemUpdated: "Đã cập nhật món",
  itemCreated: "Đã tạo món mới",
  itemNameLabel: "Tên món",
  itemNamePlaceholder: "Ví dụ: Cơm sườn cốt lết",
  selectCategoryPlaceholder: "Chọn danh mục",
  basePriceLabel: "Giá bán đã gồm thuế GTGT (VND)",
  basePriceTableLabel: "Giá bán đã gồm thuế GTGT",
  vatRateLabel: "Thuế suất GTGT",
  selectVatRatePlaceholder: "Chọn thuế suất",
  vatRateDescription:
    "Thuế GTGT đã gồm trong giá bán, không cộng thêm khi thu.",
  vatInclusivePriceSummary: (price: string, rate: string) =>
    `${price} đã gồm thuế GTGT · Thuế suất ${rate}`,
  itemSearchPlaceholder: "Tìm tên món hoặc danh mục",
  allCategories: "Tất cả danh mục",
  allStatuses: "Tất cả trạng thái",
  sortBy: "Sắp xếp",
  menuOrder: "Thứ tự thực đơn",
  nameAscending: "Tên A–Z",
  priceAscending: "Giá thấp đến cao",
  priceDescending: "Giá cao đến thấp",
  noMatchingItems: "Không tìm thấy món phù hợp",
  descriptionLabel: "Mô tả",
  descriptionPlaceholder: "Mô tả ngắn về món ăn",
  itemImageLabel: "Ảnh món",
  categoryNameRequired: "Tên danh mục không được trống",
  categoryTypeInvalid: "Loại danh mục không hợp lệ",
  editCategoryTitle: "Chỉnh sửa danh mục",
  addCategoryTitle: "Thêm danh mục mới",
  categoryUpdated: "Đã cập nhật danh mục",
  categoryCreated: "Đã tạo danh mục mới",
  categoryNameLabel: "Tên danh mục",
  categoryNamePlaceholder: "Ví dụ: Cơm tấm, Nước uống",
  selectTypePlaceholder: "Chọn loại",
  sortOrderLabel: "Thứ tự hiển thị",
  addCategory: "Thêm danh mục",
  addItem: "Thêm món",
  imageTypeError: "Chỉ chấp nhận JPG, PNG hoặc WebP.",
  imageTooLarge: "Ảnh vượt quá 5 MB.",
  imageAlt: "Ảnh món",
  changeImage: "Đổi ảnh",
  uploadImage: "Tải ảnh món (≤ 5 MB)",
  imageUploading: "Đang tải…",
  sidesSaved: "Đã lưu món ăn kèm",
  channelPricesTitle: "Giá bán trên sàn giao hàng",
  channelPricesLoading: "Đang tải giá kênh…",
  channelPricePlaceholder: "Chưa đặt",
  channelPriceModeSame: "Cùng đơn giá",
  channelPriceModeCustom: "Theo từng sàn",
  channelPriceSameHint:
    "Áp dụng cùng một mức giá cho tất cả các sàn giao hàng",
  channelPriceCustomHint: "Thiết lập đơn giá riêng cho từng sàn giao hàng",
  channelPriceQuick20: "+20% từ giá gốc",
  channelPriceQuick25: "+25% từ giá gốc",
  channelPriceQuickCalcLabel: "Tính nhanh:",
  channelPriceBaseHint: (base: string) => `Giá gốc tại quán: ${base}`,
  channelPricesSave: "Lưu giá kênh",
  channelPricesSaveFailed: "Không lưu được giá kênh",
  channelPricesSaved: "Đã lưu giá kênh giao hàng",
  channelPricesApplyAllLabel: "Đơn giá cho tất cả sàn (VND)",
  channelPricesApplyAllAction: "Lưu cùng giá cho cả 4 sàn",
  channelPricesApplyAllEmpty: "Nhập số tiền trước khi áp dụng tất cả sàn",
  channelPricesMarkupLabel: "Markup từ giá quán (%)",
  channelPricesMarkupAll: "Áp markup tất cả sàn",
  channelPricesMarkupOne: (platform: string) => `Áp markup ${platform}`,
  channelPricesMarkupFailed: "Không áp được markup giá kênh",
  channelPricesMarkupSaved: "Đã áp markup giá kênh",
} as const;

export type MenuKey = keyof typeof MENU_VI;
