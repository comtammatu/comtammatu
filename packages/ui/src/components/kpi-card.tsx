"use client";

import type { ReactNode } from "react";
import { ArrowUpRight } from "lucide-react";
import { Card, CardContent } from "./card";
import {
  Tooltip,
  TooltipContent,
  TooltipTrigger,
} from "./tooltip";
import { cn } from "../lib/utils";
import { CompareChip, type CompareDelta } from "./compare-chip";

export type KpiTone = "neutral" | "primary" | "success" | "warning" | "destructive";

export const VALUE_TONE: Record<KpiTone, string> = {
  neutral: "text-foreground",
  primary: "text-primary",
  success: "text-success",
  warning: "text-warning",
  destructive: "text-destructive",
};

export const DOT_TONE: Record<KpiTone, string> = {
  neutral: "bg-muted",
  primary: "bg-primary",
  success: "bg-success",
  warning: "bg-warning",
  destructive: "bg-destructive",
};

const LABEL_CLASS =
  "line-clamp-2 min-w-0 break-words text-xs font-medium uppercase tracking-wide text-muted-foreground";

export interface KpiCardLinkProps {
  href: string;
  className: string;
  "aria-label"?: string;
  children: ReactNode;
}

export interface KpiCardProps {
  label: string;
  value: ReactNode;
  shortValue?: ReactNode;
  valueLabel?: string;
  /** Compare delta (built via buildCompareDelta) */
  delta?: CompareDelta | null;
  /** Compare period label e.g. "so với kỳ trước" */
  compareHint?: string;
  /** Inline secondary metric e.g. "Chưa VAT · 1.234 lượt" */
  hint?: ReactNode;
  /** Hover explanation for the label; renders a dashed underline affordance */
  labelTooltip?: ReactNode;
  /** Visual tone — primary highlights hero KPI, warning/destructive flag SLA */
  tone?: KpiTone;
  /** Drill-down target. When set, the whole card is wrapped in a link. */
  href?: string;
  /** Custom link renderer for framework navigation (e.g. Next.js Link) */
  renderLink?: (props: KpiCardLinkProps) => ReactNode;
  /** Optional pre-rendered sparkline slot */
  sparklineNode?: ReactNode;
  /** Optional glyph shown top-right in a muted box (replaces the tone dot) */
  icon?: ReactNode;
  density?: "comfortable" | "compact";
  className?: string;
}

export function KpiCard({
  label,
  value,
  shortValue,
  valueLabel,
  delta,
  compareHint = "so với kỳ trước",
  hint,
  labelTooltip,
  tone = "neutral",
  href,
  renderLink,
  sparklineNode,
  icon,
  density = "comfortable",
  className,
}: KpiCardProps) {
  const isCompact = density === "compact";
  const labelNode = labelTooltip ? (
    <Tooltip>
      <TooltipTrigger
        render={
          <span
            className={cn(
              LABEL_CLASS,
              "cursor-help underline decoration-dashed decoration-muted-foreground/70 underline-offset-4",
            )}
            tabIndex={0}
          />
        }
      >
        {label}
      </TooltipTrigger>
      <TooltipContent side="top" className="max-w-sm whitespace-pre-line">
        {typeof labelTooltip === "string" ? (
          <p className="whitespace-pre-line break-words">{labelTooltip}</p>
        ) : (
          labelTooltip
        )}
      </TooltipContent>
    </Tooltip>
  ) : (
    <p className={LABEL_CLASS}>{label}</p>
  );
  const Body = (
    <CardContent
      className={cn(
        "relative flex h-full flex-col",
        isCompact ? "min-h-24 gap-2" : "min-h-32 gap-3",
      )}
    >
      <div
        className={cn(
          "flex items-start justify-between gap-2",
          !isCompact && "min-h-8",
        )}
      >
        {labelNode}
        {icon ? (
          <span
            className="flex size-8 shrink-0 items-center justify-center rounded-md bg-muted text-muted-foreground"
            aria-hidden
          >
            {icon}
          </span>
        ) : (
          <span className="flex items-center gap-1.5">
            <span
              className={cn("size-1.5 rounded-full", DOT_TONE[tone])}
              aria-hidden
            />
            {href ? (
              <ArrowUpRight
                className="size-3.5 text-muted-foreground"
                aria-hidden
              />
            ) : null}
          </span>
        )}
      </div>
      <p
        className={cn(
          "min-w-0 break-words leading-tight font-semibold tabular-nums",
          isCompact ? "text-xl" : "text-2xl",
          VALUE_TONE[tone],
        )}
        title={shortValue && typeof value === "string" ? value : undefined}
      >
        {shortValue ?? value}
      </p>
      {delta ? (
        <CompareChip label={delta.label} tone={delta.tone} hint={compareHint} />
      ) : null}
      {hint ? (
        <div className="line-clamp-2 break-words text-xs text-muted-foreground">
          {hint}
        </div>
      ) : null}
      {sparklineNode ? (
        <div className="-mx-1 -mb-1 mt-auto h-8">
          {sparklineNode}
        </div>
      ) : null}
    </CardContent>
  );

  if (!href) {
    return (
      <Card
        size={isCompact ? "sm" : "default"}
        className={cn("h-full", className)}
      >
        {Body}
      </Card>
    );
  }

  const linkClassName = cn(
    "block h-full rounded-lg focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-foreground focus-visible:ring-offset-2",
    className,
  );
  const linkAriaLabel =
    valueLabel ?? (typeof value === "string" ? `${label}: ${value}` : label);
  const cardElement = (
    <Card
      size={isCompact ? "sm" : "default"}
      className="h-full transition-[background-color,box-shadow] hover:bg-muted/50 hover:shadow-effect-card-hover"
    >
      {Body}
    </Card>
  );

  if (renderLink) {
    return renderLink({
      href,
      className: linkClassName,
      "aria-label": linkAriaLabel,
      children: cardElement,
    });
  }

  return (
    <a
      href={href}
      className={linkClassName}
      aria-label={linkAriaLabel}
    >
      {cardElement}
    </a>
  );
}
