...
function Table({ className, ...props }: React.ComponentProps<"table">) {
  return (
    <div
      ...
      className="relative w-full overflow-x-auto rounded-3xl border border-border/70 bg-background/80 shadow-sm backdrop-blur"
    >
      <table
        data-slot="table"
...
function TableRow({ className, ...props }: React.ComponentProps<"tr">) {
  return (
    <tr
      data-slot="table-row"
      className={cn(
        "border-b transition-colors hover:bg-muted/35 has-aria-expanded:bg-muted/40 data-[state=selected]:bg-muted/45",
        className
      )}
      {...props}
    />
