...
// Presentation Contract R3): management-density `size="sm"`/`size="xs"` on
...
// branch — shrink-only, so management-density buttons in these files can only
...
for (const relPath of OPERATOR_EMBEDDED_BUTTON_DENSITY_FILES) {
  ...
  if (count > allowed) {
    failures.push(
      `operator-embedded-button-density: ${relPath} has ${count} management-density Button(s) (size="sm"/"xs"), allowed ${allowed}. Operator-plane primary actions use size={embedded ? "touch" : "sm"} (page-archetypes.md § Operator Embedded Presentation Contract R3).`,
    );
  }
}
