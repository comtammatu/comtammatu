...
const checks = [
  ...
  },
  {
    ...
    description:
      "Branch runtime, Operations, and employee-lib surfaces must not import or render Management/Admin Dashboard chrome. Use the operator layout, AppHeader/AppBottomNav, EmployeePage, or embedded PageContent branches instead.",
    roots: [
      {
  ...
  },
  {
    ...
    description:
      "Branch operator routes must not link or redirect into Admin Dashboard route roots; keep work inside /br/[branchId] or a shared non-Admin Dashboard surface.",
    roots: [
      {
...
for (const file of allPageFiles) {
  ...
  if (
    ...
    failures.push(
      `page-archetype: ${file} is a DOC-WORKFLOW page without an approved frame in the page or its direct client owner. Admin Dashboard uses DocumentFormFrame; Branch touch uses BranchOperatorPage with workflow-native progressive disclosure and actions (docs/spec/page-archetypes.md § DOC-WORKFLOW).`,
    );
  }
}
...
// opening tag in that directory is the Admin Dashboard-plane LIST shell. Returns the set
...
// `content` block is often shared with the Admin Dashboard plane), so this ratchets
